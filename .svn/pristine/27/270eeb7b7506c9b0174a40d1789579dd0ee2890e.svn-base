...
--------------------
01b,17mar08,mdg Updated for the mv7100.
...
IMPORT STATUS sysMpc8641GetSpdData (UINT32, UINT32, UINT32, UCHAR *);
...
void sysStatusRegShow (void)
    ...
    (int)BRD_SYSTEM_STATUS_REG, regVal);
...
void sysStatusIndicatorRegShow (void)
    ...
    printf ("System Status Indicator Register @ 0x%x = 0x%x\r\n",
            (int)BRD_SYSTEM_STATUS_INDICATOR_REG, regVal);

    printf ("    [1] User 1 LED is ");

    if (((regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1R_LED_MASK) == 0) &&
        ((regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1Y_LED_MASK) == 0))
        {
        printf ("OFF.\r\n");
        }
    else if (((regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1R_LED_MASK) == 1) &&
        ((regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1Y_LED_MASK) == 0))
        {
        printf ("RED.\r\n");
        }
    else if (((regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1R_LED_MASK) == 0) &&
        (regVal & BRD_SYSTEM_STATUS_INDICATOR_USR1Y_LED_MASK))
        {
        printf ("YELLOW.\r\n");
        }
    else
...
/*******************************************************************************
*
* sysInterruptStatusRegShow - Show the system interrupt status registers
*
* This function displays to the console the system interrupt status
* registers.
*
* RETURNS: NA
*/

void sysInterruptStatusRegShow (void)
    {
    UINT8 regVal1 = *(UINT8 *)BRD_INTERRUPT_REG_1;
    UINT8 regVal2 = *(UINT8 *)BRD_INTERRUPT_REG_2;

    printf ("Interrupt status Register 1 @ 0x%x = 0x%x\r\n",
            (int)BRD_INTERRUPT_REG_1, regVal1);

    printf ("Interrupt status Register 2 @ 0x%x = 0x%x\r\n",
            (int)BRD_INTERRUPT_REG_2, regVal2);
...
void sysFlashStatusRegShow (void)
    ...
    printf ("System Flash Status Register @ 0x%x = 0x%x\r\n",
            (int)BRD_FLASH_CONTROL_REG, regVal);
...
void sysPresenceDetectRegShow (void)
    ...
    printf ("Presence Detect Register @ 0x%x = 0x%x\r\n",
            (int)BRD_PRESENCE_DETECT_REG, regVal);
...
/*******************************************************************************
*
* sysMpc8641SpdShow - Display the SPD values.
*
...
STATUS sysMpc8641SpdShow ()
    {
    UINT32 spdByte;                        /* SPD byte index ctr */
    UCHAR  spdData[SPD_SIZE];              /* SPD data */
    STATUS status = OK;

    status = sysMpc8641GetSpdData (SPD1_EEPROM_I2C_ADDR, 0, SPD_SIZE, spdData);

    if (status == OK)
        {
        printf("I2C Address: %X\n\r", SPD1_EEPROM_I2C_ADDR);
        printf("---------------\r\n");
    ...
    status = sysMpc8641GetSpdData (SPD2_EEPROM_I2C_ADDR, 0, SPD_SIZE, spdData);

    if (status == OK)
        {
        printf("I2C Address: %X\n\r", SPD2_EEPROM_I2C_ADDR);
        printf("---------------\r\n");
        for (spdByte = 0; spdByte < SPD_SIZE; spdByte++)
            {
            printf("Byte %d = 0x%02X\n\r", spdByte, spdData[spdByte]);
