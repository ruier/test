...
--------------------
01b,15apr09,rgo  Replace logMsg with logMsg function pointer(wind00121177)
...
END_OBJ * sysMotTsecEndLoad
    ...
    */
	...
	if (sysNetMacNVRamAddrGet ("mottsec", unit, enet, MAC_ADRS_LEN) == ERROR)
		...
		_func_logMsg ("Error: motTsecEndLoad  illegal MAC address - use M command\n",
			       0, 0, 0, 0, 0, 0);
	    return(NULL);
	    }
	    ...
	    if( _func_logMsg != NULL )
		_func_logMsg ("Error: motTsecEndLoad  failed to load driver\n",
			       0, 0, 0, 0, 0, 0);
#if 0
        else
	    if( _func_logMsg != NULL )
		_func_logMsg ("Load successful: 0x%08x\n", 
                               (int)pEnd, 0, 0, 0, 0, 0);
#endif
