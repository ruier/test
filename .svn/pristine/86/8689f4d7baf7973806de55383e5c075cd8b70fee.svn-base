...
--------------------
01e,23mar09,mdg  Corrected parameter checking.
01d,05mar09,mdg  Added monitor task to offload interrupts so that
                  I2C calls are not made in interrupt context.
01c,23oct08,mdg  Fixed length of local sting variables.
01b,17mar08,mdg  Fixed diab warning.
...
#define DS1375_TASK_PRIORITY    100
#define DS1375_STACK_SIZE       4096

#define IS_LEAP_YR(year)  (((year % 4 == 0) && (year % 100 != 0)) \
                           || year % 400 == 0)
...
LOCAL BOOL verbose = TRUE;          /* control DEBUG error statements */
...
LOCAL void   sysDs1375IntEnable();
LOCAL void   sysDs1375IntDisable();
...
LOCAL STATUS sysDs1375AlarmParamsCheck
    ...
    case ALARM_EVERY_DAY:
        ...
        if ((alarm_time->hour < 0) || (alarm_time->hour > 23))
                ...
                if (verbose)
                    {
                    logMsg("ERROR: Hour - 0 through 23\n\r",0,0,0,0,0,0);
    ...
    case ALARM_EVERY_MONTH:
        if((alarm_time->day_of_month < 1) ||
           (alarm_time->day_of_month > 31))
            {
            if (verbose)
                {
...
LOCAL STATUS sysDs1375RtcParamsCheck
    ...
    case 4: case 6: case 9: case 11:
        if((rtc_time->day_of_month < 1) || (rtc_time->day_of_month > 30))
            {
            if (verbose)
                {
        ...
        break;
    case 2:
        if((rtc_time->day_of_month < 1) ||
           (rtc_time->day_of_month > (IS_LEAP_YR (rtc_time->year) ? 29 : 28)))
            {
            if (verbose)
                {
                logMsg("ERROR: Day - 1 through %d\n\r",
                       (IS_LEAP_YR (rtc_time->year) ? 29 : 28),0,0,0,0,0);
                logMsg("Entered: %d\n\r", rtc_time->day_of_month,0,0,0,0,0);
                }
            return (ERROR);
            }
...
*
* This routine is the main interrupt handler for the DS1375.  It passes
* processing off to the monitor task.
*
* RETURNS: N/A
*/
...
LOCAL void sysDs1375Int (void)
    ...
    if (((*(UCHAR *)(BRD_INTERRUPT_REG_2)) &
          (BRD_INTERRUPT_REG_2_RTC | BRD_INTERRUPT_REG_2_RTC_MASK)) ==
          BRD_INTERRUPT_REG_2_RTC)
        {
        semGive(sysDs1375Reg.semIntId);

        /* Disable the interrupt since we haven't cleared it */
...
/*******************************************************************************
*
* sysDs1375Mon - monitor task routine to process alarms in task context
*
* This routine is the routine ran by the tDsMon task. When an alarm
...
LOCAL void sysDs1375Mon(void)
    ...
    FOREVER
        {
        semTake(sysDs1375Reg.semIntId , WAIT_FOREVER);
        ...
        /* Re-enable interrupts now that we have cleared them */
...
LOCAL STATUS sysDs1375Init (void)
    ...
    sysDs1375Reg.semIntId = semBCreate (SEM_Q_PRIORITY, SEM_EMPTY);
    ...
    if (status != ERROR)
        ...
        if(taskSpawn("tDsMon",DS1375_TASK_PRIORITY,0,
                     DS1375_STACK_SIZE,(FUNCPTR) sysDs1375Mon,
...
/*******************************************************************************
*
* sysDs1375IntDisable - disable DS1375 interrupts
*
* This routine will disable DS1375 interrupts.
*
* RETURNS: N/A
*/

LOCAL void sysDs1375IntDisable (void)
    {
    UCHAR temp = 0;

    /* set TSTAT_MASK bit in system status register 2 */

    *(UCHAR *)(BRD_INTERRUPT_REG_2) |= BRD_INTERRUPT_REG_2_RTC_MASK;
...
/*******************************************************************************
*
* sysDs1375IntEnable - enable DS1375 interrupts
*
* This routine will enable DS1375 interrupts.
*
* RETURNS: N/A
*/

LOCAL void sysDs1375IntEnable (void)
    {
    UCHAR temp = 0;
...
/*******************************************************************************
*
* sysDs1375RtcPrompt - set the real time clock interactively from the shell
*
* This function sets the time of the real-time clock interactively
* from the shell.
*
* NOTE: This is a wrapper function to timeSet() using device naming convention.
*
* RETURNS: OK, or ERROR if writing to the real-time clock fails.
*/

STATUS sysDs1375RtcPrompt (void)
    {
    return (timeSet());
    }
