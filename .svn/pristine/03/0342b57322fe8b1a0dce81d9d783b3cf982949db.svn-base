 ...
 *****************************************************************************/
 STATUS fsGetFreeEnt(FS_VOL_S  *vpstVol,UINT32  *vpuiClust)
 {
    ...
    UINT32 uiEnt = 0;
	UINT32 uiVolId = 0;
    static UINT32 auiLastFree[FS_MAX_VOL] = {0};

	assert(vpstVol->uiVolId < FS_MAX_VOL);
	uiVolId = vpstVol->uiVolId;
    for(uiLoop = (vpstVol->uiRootClu + 1);uiLoop < vpstVol->uiVolClu;uiLoop++)
    {
        auiLastFree[uiVolId] = auiLastFree[uiVolId] % vpstVol->uiVolClu;
        if(auiLastFree[uiVolId] < vpstVol->uiRootClu + 1)
        {
            auiLastFree[uiVolId] = vpstVol->uiRootClu + 1;
        }
        if(FS_CLUST_EOF == auiLastFree[uiVolId])
        {
            FS_LOG(FS_DBG_ERR,"fsGetFreeEnt last free clu invalid\n",
                   auiLastFree[uiVolId],0,0,0,0,0);
        }
        assert(auiLastFree[uiVolId] > 0);
        if(OK != fsReadEnt(vpstVol,auiLastFree[uiVolId],&uiEnt))
        {
        	assert(1);
            return ERROR;
        }
        ...
        {
            /*FS_CACHE_WR_BACK*/
            if(OK != fsWriteEnt(vpstVol,auiLastFree[uiVolId],FS_CLUST_EOF,FS_CACHE_WR_BACK))
            {
               return ERROR;
            }
			assert(auiLastFree[uiVolId] > 0);
            *vpuiClust = auiLastFree[uiVolId];
             auiLastFree[uiVolId]++;
            return OK;
        }
        auiLastFree[uiVolId]++;
