...
01b,02jul03,yyz Fixed the minor bug in sysMotI2cWrite().
01a,02oct02,scb Adapted from hxeb100 base (ver 01b).
*/
...
prevent it from being compressed during kernel generation.  This means that
there are some coding restrictions: no switch statements, no initialized
data, and no global data.
        ...
        if (*p == devAddr)
            ...
            for (x = 0; x <= numBytes / pageBytes; x++)
                ...
                if (devAddrBytes == I2C_SINGLE_BYTE_ADDR)
                    {
                    if (sysMv64260I2cStart(base) == ERROR)
                        return(ERROR);
                    addrByte = (UCHAR)(I2C_ADDR_MUNGE(devAddr,offset));
                    if (sysMv64260I2cWrite(base,&addrByte) == ERROR)
                        return(ERROR);
                    if (sysMv64260I2cAckIn(base) == ERROR)
                        return(ERROR);
                ...
                for (byteCount = 0; byteCount < pageBytes && 
                    ...
                    {
                    if (sysMv64260I2cWrite(base,pBfr) == ERROR)
                        return(ERROR);
                ...
                if (sysMv64260I2cAckIn(base) == ERROR)
                    return(ERROR);
                if (sysMv64260I2cStop(base) == ERROR)
                    return(ERROR);
...
STATUS sysMotI2cRead
    ...
    for ( ; numBytes != 0; --numBytes)
        ...
        if (numBytes != 1)
            {
            if (sysMv64260I2cRead(base,pBfr,NOT_LAST_BYTE) == ERROR)
                return(ERROR);
            }
        else
            {
            if (sysMv64260I2cRead(base,pBfr,LAST_BYTE) == ERROR)
                return(ERROR);
            }
        pBfr++; /* increment pointer to next byte */
        }
