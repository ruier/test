/* sysNet.c - LSI ACP34XX Board system-dependent Network Library */
...
--------------------
01c,25sep12,x_s  fixed the building warning issue. (WIND00378291)
01b,23feb11,x_z  added fault treatment fot sysNvRamGet().
01a,21jun10,x_z  created from amcc_kilauea/sysNet.c v01a.
*/
...
const char * sysNetDevName[MAX_MAC_DEVS] = {"lsiMac"};
...
STATUS sysNetMacAddrSet
    ...
    if (sysNvRamGet (pEnet, ifMacAddrLen, NV_MAC_ADRS_OFFSET + offset) == ERROR)
        return ERROR;
    ...
    if (sysNvRamSet ((char *)ifMacAddr, ifMacAddrLen, NV_MAC_ADRS_OFFSET + offset)
        == ERROR)
        return ERROR;
...
STATUS sysNetMacNVRamAddrGet
    ...
    if ((sysNvRamGet (pEnet, ifMacAddrLen, NV_MAC_ADRS_OFFSET + offset) ==
        ERROR) ||
        (memcmp (pEnet, sysInvalidAddr[0], MAC_ADRS_LEN) == 0) ||
        (memcmp (pEnet, sysInvalidAddr[1], MAC_ADRS_LEN) == 0))
        {
        printf ("Failed to fetch MAC address from NVRAM, so just use default value.\n");
        bcopyBytes ((char *) &glbEnetAddr[ifUnit], pEnet, MAC_ADRS_LEN);
        return (ERROR);
        }
