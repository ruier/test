...
--------------------
19w,17apr13,elp  Added RUNTIME_CREATION_TIME parameter (WIND00295940).
19v,10apr13,cww  Added constant for task create hook table size (WIND00395617)
19v,31jan13,zly  Add DOSFS_COMPAT_NT DOSFS_BLOCKS_PER_DATA_DIR_CACHE_GROUP 
                 DOSFS_BLOCKS_PER_FAT_CACHE_GROUP DOSFS_DEFAULT_VOL_BIO_BUFFER_SIZE
                 DOSFS_PRTMSG_LEVEL
19u,18jan13,x_s  Added commented macro NV_MAC_ADRS_OFFSET for BSP to 
                 use in the future.
19t,21aug12,cwl  Increase ISR_STACK_SIZE for LP64. (WIND00367947)
19s,12jul12,l_z  Add ROMSTART_BOOT_CLEAR by default. (WIND00318017)
19r,04jun12,zly  Fix WIND00342009: add comments for DOSFS_DEFAULT_CREATE_OPTIONS
19q,15may12,jmp  CQID: Fix WIND00309977 : Removed COPROC options from
                 SHELL_TASK_OPTIONS & SHELL_SPAWNED_TASK_OPTIONS parameters.
19p,09mar12,jdw  Fix ARM Thumb2 to exclude INCLUDE_SW_FP by default
19o,26apr12,wyt  Updated ROOT_STACK_SIZE for LP64. (WIND00272767)
19n,20apr12,jb   WIND00314441 - Auxclk interrupt ends up on the exception stack
19m,16jan12,j_b  Remove RTP_OVERLAPPED_ADDRESS_SPACE as default for
                 ARMv4/5/XScale due to performance (WIND00246765)
19l,30dec11,zly  WIND00217466: add a direct mode for xbd. 
19k,07oct11,rbc  WIND00308927 - Remove INCLUDE_CPU_PWR_MGMT
19j,28jul11,rbc  WIND00238999 - Add VSB _WRS_CONFIG_PWR_MGMT
19i,28jun11,jxu  Added PIPE_MAX_OPEN_FDS (WIND00271309).
19h,12apr11,j_l  IA Multi-stage boot support (WIND00263692).
19g,18mar11,jmp  renamed SHELL_SP_* parameters to SHELL_SPAWNED_TASK_*.
19f,16mar11,jmp  Added SHELL_SP_PRIORITY, SHELL_SP_OPTIONS,
                 SHELL_SP_STACK_SIZE parameters (CQ:WIND00235451).
19e,07feb11,rlp  Enabled system mode agent for LP64.
19d,12nov10,jb   Fix for Defect WIND00236404 - USB included
19c,22oct10,rlp  Added INCLUDE_SYSDBG_FLAG component.
19b,18oct10,cww  Updated ISR_STACK_SIZE and TASK_USER_EXC_STACK_SIZE for LP64
19a,01oct10,gls  added INCLUDE_EDR_POLICY_HOOKS (WIND00234560)
18z,30sep10,ggz  Add LOGIN_PASSWORD_SALT
18z,09sep10,ggz  Replace LOGIN_PASSWORD with sha256 hashed password
18y,08sep10,pcs  Add macro GLOBAL_VIRT_MEM_REGION_SIZE.
18y,08sep10,s_s  KERNEL_HEAP_SIZE now uses the macro FREE_MEM_START_ADRS to 
                 get its value (CQ: WIND00214548) 
18y,24aug10,slk  only define AIM_MMU_INIT_MEM_POOL_SIZE if CDF_OVERRIDE not
                 defined
18x,30jul10,slk  redefine SM_ANCHOR_ADRS, BOOT_LINE_ADRS and EXC_MSG_ADRS for
                 MIPS LMM support
18x,29jul10,jmp  Changed WDB_POOL_SIZE to 0 when system mode agent is not
		 supported.
18w,28jul10,j_l  Modified WDB_POOL_SIZE for LP64.  Does not use sysMemTop().
18w,27jul10,pgh  Add ELF memory map component to MIPS bootrom.
18v,10jul10,slk  add AIM MMU pool configuration parameters for MIPS
18v,22jun10,jb   Adding BOOT_USB_POST_MS_BULKONLY_INIT_DELAY for IA USB nvram
                 support
18u,07may10,jmp  Removed WDB_REGS_SIZE parameter (Coproc register size is now
                 dynamically computed, except in WDB Pre-kernel init config
		 where it is limited to WDB_MTU) (CQ:WIND00200823).
18x,15mar10,rec  remove power management framework configuration
18w,29apr10,sem  Adjust KERNEL_COMMON_HEAP_INIT_SIZE default value
18v,27apr10,sem  Add E820 auto-size storage address
18w,15mar10,rec  remove power management framework configuration
18v,01apr10,jpb  Added INCLUDE_USER_RESERVED_MEMORY
18u,29mar10,rlp  Disabled system mode agent for VxWorks 64-bit.
18t,25feb10,rlp  Added VxDBG component.
18s,04feb10,kk   switch to using overlap for default (REQ:WIND00142021)
18r,20jan10,zl   added INCLUDE_MMAP
18q,29dec09,h_k  undefed INCLUDE_TASK_VARS in LP64 build.
18p,04nov09,pcs  Updated to reflect the name changes to the 64bit heaps.
18y,23oct09,s_s  Added kprintf defines to support BSP build (WIND00186733)
18o,08sep09,pcs  Updated to add support for multiple heaps for 64bit vxWorks.
...
#define INCLUDE_EDR_POLICY_HOOKS /* ED&R policy hooks */
...
#define INCLUDE_KERNEL_PROXIMITY_HEAP /* include kernel proximity heap. */
#define INCLUDE_KERNEL_COMMON_HEAP    /* include kernel common heap. */
#endif
...
#define	INCLUDE_VXDBG		/* VxWorks debug library */
...
#define INCLUDE_SYSDBG_FLAG	/* include system debug flag support */
...
#if     (CPU_FAMILY==MIPS)
#define INCLUDE_BOOT_ELF         /* Boot ELF file loader */
#define INCLUDE_ELF_MEMORY_MAP   /* ELF loader memory mapping */
...
/* include light power management if supported by architecture */
...
#define INCLUDE_MMAP		/* support for mmap() and associated routines */

#define INCLUDE_USER_RESERVED_MEMORY /* user-reserved memory */
...
/*
 * Overlapped RTP virtual address space is disabled by default for ARM CPUs 
 * with virtually-tagged caches due to performance degradation (WIND00246765).
 */
...
#define	RTP_OVERLAPPED_ADDRESS_SPACE	TRUE /* Use overlap virtual address
...
/* in AMP mode INCLUDE_WDB_SYS results in project errors */
...
/*
 * Default kernel shell tasks options:
 * This parameter defines options for kernel shell tasks initialization.
 * Supported coprocessor options such as VX_FP_TASK are automatically added
 * by the coprocessor discovery facility and do not need to be specified.
 */
...
/*
 * Default parameters for task spawned by sp(), period() and repeat()
 * shell routines:
 */

#define SHELL_SPAWNED_TASK_STACK_SIZE	20000	/* Stack size of tasks */
						/* spawned by the shell */
#define	SHELL_SPAWNED_TASK_PRIORITY	100	/* Priority of tasks spawned */
						/* by the shell */
#define SHELL_SPAWNED_TASK_OPTIONS	(VX_COPROCS_ALL_TASK | VX_STDIO)
						/* Options of tasks spawned */
						/* by the shell */
...
#else
#ifdef _WRS_CONFIG_LP64
#define TASK_USER_EXC_STACK_SIZE       16384 /* usr task exc stack size */
#else
...
/* runtime creation date and time */

#ifndef RUNTIME_CREATION_DATE
#define RUNTIME_CREATION_TIME	__DATE__" "__TIME__
#else
#define RUNTIME_CREATION_TIME	RUNTIME_CREATION_DATE
...
/* 
 * Please refer to 'DevCreateOptions' section in dosFsLib.h 
 * and dosFsCommon.h for detailed options set 
 */
...
#ifdef _WRS_CONFIG_LP64
/* Default values for Kernel Proximity Heap. */

#define KERNEL_PROXIMITY_HEAP_SIZE	0x10000000

/* Default values for Kernel Common Heap. */

#define KERNEL_COMMON_HEAP_INIT_SIZE	0x20000000
...
/* VxWorks debug library default configuration */
...
/* This value is defined here to reserve 
   the memory offset in low memory.
   It is codependent with the same value
   defined in vxBiosE820LibSP.h.  Both values
   must match.
*/

#define BIOS_E820_DATA_OFFSET   0x4000
...
/*
 *  The MAC address is stored in non-volatile RAM at the default offset 0x200.  
 *  Currently this definition is commented out due to many BSPs have this definition 
 *  on config.h with different offset value. In the future, all the BSPs would move
 *  to use the usrMacAddr.c and remove the definition on config.h, the below macro 
 *  would be un-commented out.
 */
...
/*
 * At this point INCLUDE_LARGE_MEMORY_MODEL will only be set correctly
 * for command line builds in a BSP so these defines are specific
 * for this type of build.  These same defines are added to 
 * config/comps/vxWorks/arch/mips/20bsp.cdf for project builds.  The
 * defines in 20bsp.cdf also have defines to support AMP builds.
 */

#if !defined(CDF_OVERRIDE)      /* don't scan when VIP is created */
#if defined(INCLUDE_LARGE_MEMORY_MODEL)
#define BOOT_LINE_ADRS    ((char *)(K0BASE+BOOT_LINE_OFFSET))
#define SM_ANCHOR_ADRS    ((char *)(K0BASE+SM_ANCHOR_OFFSET))
#define EXC_MSG_ADRS      ((char *)(K0BASE+EXC_MSG_OFFSET))
#else /* !INCLUDE_LARGE_MEMORY_MODEL */
#define BOOT_LINE_ADRS    ((char *)(KM_TO_K0(LOCAL_MEM_LOCAL_ADRS+BOOT_LINE_OFFSET)))
#define SM_ANCHOR_ADRS    ((char *)(KM_TO_K0(LOCAL_MEM_LOCAL_ADRS+SM_ANCHOR_OFFSET)))
#define EXC_MSG_ADRS      ((char *)(KM_TO_K0(LOCAL_MEM_LOCAL_ADRS+EXC_MSG_OFFSET)))
#endif /* INCLUDE_LARGE_MEMORY_MODEL */
#endif /* !defined(CDF_OVERRIDE) */
...
#define	ROMSTART_BOOT_CLEAR /* clear memory when booting */
...
#ifndef LOGIN_PASSWORD_SALT
/* password salt */
#define LOGIN_PASSWORD_SALT "BFqADK/VLEk="
#endif  /* LOGIN_PASSWORD_SALT */
...
/* AIM MMU memory partition virtual address and size */

#if (CPU_FAMILY == MIPS)
#ifndef CDF_OVERRIDE
#define AIM_MMU_INIT_MEM_POOL_SIZE      0x00200000
#define AIM_MMU_INIT_MEM_POOL_VIRT_ADRS 0x78000000
#endif /* CDF_OVERRIDE */
#endif /* (CPU_FAMILY == MIPS) */
