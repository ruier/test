...
IMPORT STATUS mpc8640DI2CWrite(MPC8640D_I2C_DRV_CTRL * pDrvCtrl,UINT8 addr7bit,
	                           UINT8 * data,UINT32  nbytes,UINT32  options,
	                           int 	timeout);
	                           
IMPORT STATUS mpc8640DI2CRead(MPC8640D_I2C_DRV_CTRL *pDrvCtrl, 
                           	  UINT8  addr7bit,UINT8 *data,UINT32 nbytes,
                           	  UINT32 options,int timeout);
...
{
    MPC8640D_I2C_DRV_CTRL  *pstI2CDrv;
    UINT32                  uiInitFlag;
}I2C_CTRL_S;
...
UINT8 ADT7483AWrite(UINT8 busNo,UINT8 devAddr,UINT8 regAddr,UINT8 data)
{
	...
	if(ERROR == mpc8640DI2CWrite(gstI2CCtrl[busNo].pstI2CDrv,devAddr,buf,2,0,
	                             0x100000))
	{
        return 0xff;
	}
...
UINT8 ADT7483ARead(UINT8 busNo,UINT8 devAddr,UINT8 regAddr)
{
	...
	if(ERROR == mpc8640DI2CWrite(gstI2CCtrl[busNo].pstI2CDrv,devAddr,buf,1,0,
	                             0x100000))
	{
        return 0xff;
	}
	...
	if(ERROR == mpc8640DI2CRead(gstI2CCtrl[busNo].pstI2CDrv,devAddr,&buf[1],1,0,
	                            0x100000))
	{
        return 0xff;
	}
...
UINT16 sysGetCPUATemp(UINT8  busNo)
{
	UINT8  cpuTempLow = 0;
	UINT8  cpuTempHigh = 0;
...
UINT16 sysGetCPUBTemp(UINT8  busNo)
{
    UINT8  cpuTempLow = 0;
    UINT8  cpuTempHigh = 0;
...
UINT16 sysGetFPGATemp(UINT8  busNo)
{
    UINT8  cpuTempLow = 0;
    UINT8  cpuTempHigh = 0;
...
STATUS tempModuleInit(UINT8 busNo)
{
    ...
    if(0x0 == busNo)
    {
        strcpy(buf,"0:0xe0003000");
    }
    else
    {
        strcpy(buf,"0:0xe0003100");
    }
    
    gstI2CCtrl[busNo].pstI2CDrv = (MPC8640D_I2C_DRV_CTRL*)mpc8640DI2CDrvLoad(buf);
...
void cpuTempMonitor(UINT8  busNo)
{    
	INT32 fpgaTempLast = 0;
	UINT8 fpgaTemp = 0;

	fpgaTemp = sysGetFPGATemp(busNo);
...
STATUS tempMonitorInit(UINT8 busNo)
{
    char   buf[64] = {0};

	guiWarnThres = 0x5a;
	guiHaltThres = 0x64; 
	taskSpawn("tempMon",105,0,0x10000,cpuTempMonitor,busNo,0,0,0,0,0,0,0,0,0);
...
STATUS i2cWriteBuf(UINT8 busNo,UINT8 devAddr,UINT8 *data,UINT32 nbytes)
{
	...
	iRet = mpc8640DI2CWrite(gstI2CCtrl[busNo].pstI2CDrv,devAddr,data,nbytes,0,
	                             0x100000);
