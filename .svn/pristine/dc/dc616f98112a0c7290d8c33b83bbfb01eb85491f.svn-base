...
--------------------
01c,17apr12,x_z  disabled debug log by default and corrected some debug information.
01b,21mar12,x_z  removed unnecessary enable operation for sysTffsUnlock() to fix
                 TrueFFS issue; 
                 added enable/disable operations for reset operation for 
                 sysTffsIdentify();
                 removed unnecessary PRJ_BUILD macro.
01a,27feb12,x_z  written for TrueFFS interface to vxBus NOR-Flash driver.
*/
...
Note: If TFFS_BOOT_LOCK is defined and available, sysTffsBootUnLock() must be
called first before tffsBootImagePut() is used and sysTffsBootLock() should be
called to protect the boot region later.
...
sysTffsShow() is supplied and can be used to show the TrueFFS partation
information.
...
#   define TFFS_DBG_MSG(fmt, a, b, c, d, e, f)           \
        if (_func_logMsg != NULL)                       \
            _func_logMsg ("sysTffs: "fmt, a, b, c, d, e, f)
#else /* !FLASH_TFFS_DBG */
    ...
    for (i = 0; (i < FLASH_TFFS_PART_NUM) && (noOfDrives < FLASH_TFFS_PART_NUM);
        ...
        if (pFlashDev != NULL)
            ...
            if (((tffsPartTbl[i].pFlashChip->uCapability &
                FLASH_CHIP_CAP_RD) == 0) ||
                (tffsPartTbl[i].pFlashChip->flashOPs.read == NULL) ||
                ((tffsPartTbl[i].pFlashChip->uCapability &
                FLASH_CHIP_CAP_WR) == 0) ||
                (tffsPartTbl[i].pFlashChip->flashOPs.write == NULL) ||
                ((tffsPartTbl[i].pFlashChip->uCapability &
                FLASH_CHIP_CAP_BLKERA) == 0) ||
                (tffsPartTbl[i].pFlashChip->flashOPs.blkErase == NULL))
                {
            ...
            if (((tffsPartTbl[i].pFlashChip->uCapability &
                ...
                (tffsPartTbl[i].pFlashChip->flashOPs.blkLock != NULL))
                {
                ...
                if (((tffsPartTbl[i].pFlashChip->uCapability &
                    ...
                    (tffsPartTbl[i].pFlashChip->flashOPs.blkLockQry != NULL) &&
...
void sysTffsShow (void)
    ...
    for (i = 0; i < FLASH_TFFS_PART_NUM; i++)
        ...
        printf ("TrueFFS Partition %d - %s (%s)\n",
                i, tffsPartTbl[i].pPartName,
                tffsPartTbl[i].partReady ? "Ready" : "Not ready");
