...
--------------------
02m,27jun12,l_z  repalce INCLUDE_AMP with INCLUDE_AMP_CPU
02l,12jan12,agf  remove exIrqWkrd from epic resource, deprecated in driver
                 (WIND00176018)
02k,09feb10,liu  add _WRS_VX_AMP for compile error. (WIND00192173)
02j,26jun09,h_w  remove MCB, MOB and VxBus references for MIPC
02i,01apr09,rgo  Use SM_BASE_ADRS to define MOB/DHSM base Adrs(wind00145934).
02h,05jan09,b_m  remove openPicTimer vectors.
02g,30oct08,ebh  Fix anchor base for MOB_PLB_1
02f,24oct08,ebh  Change of MOB name for 8572
02e,23oct08,d_l  Add OpenPic timer vector support. (WIND00140273)
02d,17oct08,wap  Correct PCI bus configuration so that interrupts on the
                 32-bit PCI slots work crrectly (WIND00136938)
02c,14oct08,ebh  Add in comment for MOB_8572_ANCHOR_BASE
02b,30sep08,dtr  Code review fixes and tidyup.
02a,04sep08,dtr  Remove AMP_CORE1, sysDeviceFilter used to remove unwanted
                 drivers per CPU.
01z,01sep08,ebh  Move include of intCtlr/vxbEpicIntCtlr.h back to config.h
                 (VxMP use)
01y,29aug08,kab  Change _WRS_VX_SMP to _WRS_CONFIG_SMP
01x,13aug08,dtr  Add support for MIPC.
01w,04aug08,dtr  Add l2disable parameter for TSEC.
01v,08jul08,pmr  remove previous ULI code and dynamic interrupt table
01u,08may08,pmr  add support for ULI PCI bridge.
01t,29apr08,dlk  Support vxbEtsecEnd2 driver. Make (e)tsec entries in
                 hcfDeviceList be present unconditionally.
01s,04mar08,h_k  swap the init order between epic and ppcIntCtlr.
01r,05sep07,bwa  removed DSHM_SM_POOL_BASE duplicate definition (99879)
01q,14aug07,dtr  Make resources configurable.
01p,15aug07,h_k  added missing comma for INCLUDE_RAPIDIO_BUS.
01o,01aug07,h_k  added LAWAR_ENABLE and LAWBAR_ADRS_SHIFT.
01n,02jul07,dtr  Support MSI (dynamically allocated interrupts).
01n,02jul07,wap  Switch to new VxBus TSEC drivers
01m,07jun07,pmr  EPIC regbase change
01l,25may07,bwa  added distributed shared memory virtual devices.
01k,13apr07,pch  add entries for vxbOpenPicTimer
01j,05apr07,tor  vxIpi support
01i,19Mar07,tor  update interrupt structures
01h,16feb07,dtr  Make inbound window prefetchable.
01g,07feb07,dtr  PCI and timer support.
01h,10jan07,pch  update for unified vxbPpcIntCtlr.c
01g,14Nov06,tor  Integrated VxBus interrupt controller
01f,07jun06,dtr  Add in vxBus ns16550 driver support.
01e,06mar06,dtr  Initial support for vxBus created from cds8548.
*/
...
/* FIXME: move file to target/h/hwif/vxbus, or eliminate its use here */
...
/*
 * Note: the MPC8641D has two PCI Express channels, PCIEXA and PCIEXB.
 * On the hpcNet8641 board, channel A is wired to a ULi bridge device
 * and has two standard 32-bit PCI slots. Channel B is connected to
 * a single x16 PCIe slot. Because it is not possible to connect a
 * PCIe device to channel A, we don't enable MSI support on it. 
 */

const struct hcfResource m85xxPci0Resources[] = {
    { VXB_REG_BASE,         HCF_RES_INT,  { (void *)PCIEX_REGBASE } },
...
/*
 * On the hpc8641 board, PHYs are physically wired according
...
const struct hcfResource m85xxRio0Resources[] = {
    { VXB_REG_BASE, HCF_RES_INT, { (void *)RAPIDIO_BASE }},
...
#define DSHM_ANCHOR_BASE        (SM_BASE_ADRS + SM_ANCHOR_OFFSET)
