...
/*
 * Copyright (c) 2000-2001, 2003-2005, 2007 Wind River Systems, Inc.
 *
...
--------------------
01k,06apr07,l_z  Remove priority setting code in sysTffsFormat(). (WIND00066633)
01j,22jun05,pcm  removed #include "dosFsLib.h"
01i,27aug04,mil  Allowed use of boot and usr flash in same image.
01h,14oct03,mil  Updated size for flash window.
...
static FLStatus		rfaRegister (int drvNum);

/* globals */

char pcDriveNo[2] = {(char)NONE, (char)NONE};	/* drive num of the sockets */
...
static FLStatus rfaRegister
    ...
    switch (drvNum)
        {
    case 0:
        vol.window.baseAddress = BOOT_FLASH_ADRS >> 12;
        break;
    case 1:
        vol.window.baseAddress = USR_FLASH1_ADRS >> 12;
        break;
        }
...
static void rfaSetWindow
    ...
    switch (vol.volNo)
        ...
        case 0:
            vol.window.baseAddress = BOOT_FLASH_ADRS >> 12;
            flSetWindowSize (&vol, BOOT_FLASH_WINDOW_SIZE >> 12);
            break;
        case 1:
            vol.window.baseAddress = USR_FLASH1_ADRS >> 12;
            flSetWindowSize (&vol, USR_FLASH_WINDOW_SIZE >> 12);
            break;
        }
    }
...
#undef HALF_FORMAT
#ifdef  HALF_FORMAT
	{0x100000l, 10, 1, 0x10000l, NULL, {0,0,0,0}, NULL, 2, 0, NULL},
#else
    ...
    /* Format them */
    status = tffsDevFormat (drvNum, (int)&params);
