...
extern STATUS fsFindFDT(FS_VOL_S *,CHAR *,UINT32 ,FS_FDT_INFO_S *);
extern STATUS fsGetModWrFDT(FS_VOL_S *,UINT32 ,FS_FDT_INFO_S *);
extern STATUS fsListFDT(FS_VOL_S *,UINT32 );
extern STATUS fsReadFDT(FS_VOL_S *,FS_FDT_S *, UINT32 ,UINT32 );
extern STATUS fsWriteFDT(FS_VOL_S *,FS_FDT_S *,UINT32 ,UINT32 );
extern STATUS fsGetFDTCnt(FS_VOL_S *,UINT32 ,UINT32 *);
...
FS_VOL_S* fsAllocVol(VOID)
{
    ...
    for(uiLoop = 0;uiLoop < FS_MAX_VOL;uiLoop++)
    {
        if(FS_VOL_FREE == gstFsCtrl.astVol[uiLoop].uiFlag)
        {
			...
			gstFsCtrl.astVol[uiLoop].uiVolId = uiLoop;
...
VOID fsFreeVol(FS_VOL_S *vpstVol)
{
    if(FS_VOL_FREE != vpstVol->uiFlag)
    {
		...
		memset(&vpstVol->astFd[0],0,sizeof(FS_FD_S) * FS_MAX_FD);
		free(vpstVol->pcCacheData);
		vpstVol->pcCacheData = NULL;
		vpstVol->uiMaxCacheClu = 0;
		vpstVol->uiMinCacheClu = 0;
		free(vpstVol->pcCacheMgt);
		vpstVol->pcCacheMgt = NULL;
...
STATUS fsInit(VOID)
{
    ...
    gstFsCtrl.stFDTFunc.fsListFDT = fsListFDT;
    gstFsCtrl.stFDTFunc.fsReadFDT = fsReadFDT;
    gstFsCtrl.stFDTFunc.fsWriteFDT = fsWriteFDT;
    gstFsCtrl.stFDTFunc.fsGetFDTCnt = fsGetFDTCnt;
...
STATUS fsAddVol(FS_VOL_S *vpstVol)
{
    ...
    */
	if(OK != fsCluRW(vpstVol,vpstVol->uiRootClu,vpstVol->uiCluByte,
 ...
 *****************************************************************************/
 STATUS fsMountVol(FS_VOL_S *vpstVol)
 {
     ...
     {
         FS_LOG(FS_DBG_ERR,"ios add device %p failed\n",vpstVol->aucPartName,0,0,0,0,0);
         return ERROR;
     }
