...
LOCAL void sysTempeVmeIntr (void)
	...
	p++;	/* Advance to the next entry in priority table */
     ...
     /*
     * This isn't a valid check if using a PMCSpan card. PMC devices
     * installed on the span card may share this vector.  If they do,
     * unique device level ISRs may be attached for servicing these
     * devices.  Therefore, this ISR should simply yield way to other
     * daisy chained handlers if a Tempe interrupt isn't pending.
     */
#ifdef SPAN_SUPPORT_NOT_REQUIRED
    if (dispatch == FALSE)
      {
