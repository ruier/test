...
--------------------
01b,08jan03,dtr  Modifcations required PCI outbound translation feature.
01a,04jul02,dtr  Modified for ads826x BSP from mv5100 BSP.
*/
...
END_OBJ * sysFei82557EndLoad
	...
	/* 0x40 is the number of tx and rx buffers */
	sprintf (cp, "-1:-1:0x10:0x10:0x00");
...
void sys557PciInit (void)
    ...
    for (unit = 0; unit < FEI_MAX_UNITS; unit++)
        ...
        if (pciFindDevice ((PCI_ID_I82559 & 0xffff), (PCI_ID_I82559 >> 16),
           ...
           {
	   if (pciFindDevice ((PCI_ID_I82559_IB & 0xffff), 
			      (PCI_ID_I82559_IB >> 16),
			      unit, &pciBus, &pciDevice, &pciFunc) != OK)
	   break;
	   }
...
STATUS sys557Init
    ...
    if (pReso->boardType == TYPE_PRO100B_PCI)   /* only setup once */
        ...
        pReso->timeout     = 160000;             /* Timeout for set-test.  */
        ...
        do  {
            sysUsDelay (1000); WRS_ASM("sync");       /* delay for one IO READ cycle */
