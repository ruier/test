...
--------------------
01i,17aug05,dtr  fix headers for apigen and mod specifically for ads834x BSP.
                 Pulled vxWorks 6.x version into ads834x BSP.
...
ads834x         28F640J3x       RFA             SAVE_NVRAM_REGION
                                                BUFFERED_WRITE_BROKEN
...
/***************************************************************************
* cfiscsWordErase - Erase specified number of blocks
*
* Given the first block and number of contiguous blocks to erase, this 
* routine will erase the content of the specified region. The routine does
* not support featurees like "Suspend Erase" or "Queued Erase" so the device
...
#ifdef SAVE_NVRAM_REGION
    vol.chipSize = 0x400000 - vol.erasableBlockSize;
#else
