 ...
 */
/* Copyright (C) 2007 Tundra Semiconductor */

/*
modification history
--------------------
01i,29jun08,y_w  Add support for VxWorks 6.7
01h,23oct07,y_w  Fix gnu compiler warnings.
01g,24jul07,y_w  Add support for saving MAC addresses into NVRAM.
01f,11may07,jjm  Initial version, based on IBM 750GX/GL EVB BSP
...
STATUS tsi108EnetAddrGet (TSI108_DRV_CTRL * pDrvCtrl);
STATUS tsi108EnetAddrSet (TSI108_DRV_CTRL * pDrvCtrl);

IMPORT void sysMsDelay(UINT32 delay);
IMPORT STATUS sysNetMacNVRamAddrGet (char *ifName, int ifUnit, UINT8 *ifMacAddr,
                              int ifMacAddrLen);
...
END_OBJ* tsi108EndLoad
    ...
    if (tsi108EnetAddrGet(pDrvCtrl) == ERROR)
        {
        TSI108_LOG(TSI108_DEBUG_LOAD,
            ("tsi108End: Using default MAC address\n"),1,2,3,4,5,6);
		tsi108SetMacAddress(pDrvCtrl, &(enetAddrTbl[pDrvCtrl->portNum][0]));
        tsi108EnetAddrSet(pDrvCtrl);
        }
	else
		tsi108SetMacAddress(pDrvCtrl, &(pDrvCtrl->enetAddr));
...
/***********************************************************************
*
* tsi108EnetAddrGet - get the hardware Ethernet address
*
* This routine provides the six byte Ethernet hardware address that will be
* used by each individual ethernet device unit.  This routine will get 
* the MAC address from NVRAM then copy it to pDrvCtrl->enetAddr.
*
* RETURNS: OK, or ERROR if the read MAC address is not correct.
*/
STATUS tsi108EnetAddrGet
    (
    TSI108_DRV_CTRL * pDrvCtrl
    )
    {
    ...
    status = sysNetMacNVRamAddrGet ("tsiG", pDrvCtrl->unit, eAdrs, MAC_ADRS_LEN);

    if (status == OK)
        {
        memcpy(pDrvCtrl->enetAddr,eAdrs,MAC_ADRS_LEN);
        }
...
/***********************************************************************
*
* tsi108EnetAddrSet - Set the hardware Ethernet address
*
* This routine provides the six byte Ethernet hardware address that will be
* used by each individual TSEC device unit.  This routine will copy 
* the MAC stored in pDrvCtrl->enetAddr into NVRAM.
*
* RETURNS: OK, if MAC address available, ERROR otherwise
*/
STATUS tsi108EnetAddrSet
    (
    TSI108_DRV_CTRL * pDrvCtrl
    )
    {
    ...
    status = sysNetMacAddrSet ("tsiG", pDrvCtrl->unit, pDrvCtrl->enetAddr, MAC_ADRS_LEN);
