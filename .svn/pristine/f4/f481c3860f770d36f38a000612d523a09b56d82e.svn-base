...
/*
 * Copyright (c) 1995, 1997-2000, 2004, 2008, 2012 Wind River Systems, Inc.
 *
...
--------------------
01q,22aug12,clx  Fix i8250Startup function description. (WIND00371337)
01p,30jan12,sye  Stop THR empty interrupt when there are no bytes to send,
                 SPR 74889: fix hardware flow control, fix WIND00065553: 
                 possibility of losing interrupt. (WIND00286133)
01o,02jun08,kab  CQ:WIND00124720 - Use of NULL ptr
01n,01nov04,mdo  Documentation fixes for apigen
01m,18may04,jln  fix compiler warning
01l,22oct02,scb  Fix i8250Int() to read "iid" register before "ier" to fix
                 problem in which transmit buffer empty bit never seen set.
...
RTS/CTS handshake. The function HUPCL(hang up on last close) is available.
...
*
* This routine initializes the given channel.
*
* RETURNS: N/A
*
* ERRNO
...
void i8250Int
    ...
    char ier = 0;       /* store contents of interrupt enable register */
    ...
    while (ix++ < I8250_IIR_READ_MAX)
        ...
        if (interruptID == I8250_IIR_SEOB)
            ...
            if ((pChan->getTxChar != NULL) &&
                ...
                {
                /*
                 * There are no bytes available for transmission, so stop 
                 * THR empty interrupt. Reading the IIR at the top of this 
                 * loop will clear the interrupt.
                 */
...
*
* Enable interrupt if it is in interrupt mode with no hardware flow control.
...
*
* This routine polls the given channel for input.
*
