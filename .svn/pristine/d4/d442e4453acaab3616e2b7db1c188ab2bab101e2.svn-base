...
--------------------
01b,15aug05,efb  Added RTM VPD read/init support.
01a,31jan05,cak  Ported from 01f, mv6100/sysMotVpd.c.
*/
...
#ifndef RTM_VPD_READ
    IMPORT STATUS sysMotI2cRead(UINT32, UINT32, UINT32, UCHAR *, UINT32);
    #define RTM_VPD_READ(offset, bfr, num) \
                         sysMotI2cRead(VPD_RTM_EEPROM_I2C_ADDR, (offset), \
                         VPD_BYTE_ADDR_SZ, (bfr), (num))
#endif /* RTM_VPD_READ */
...
VPD sysRtmVpd; /* RTM vital product data */

VPD_PACKET * sysVpdPkts[VPD_PKT_LIMIT] = { 0 };     /* board vpd packets */
VPD_PACKET * sysRtmVpdPkts[VPD_PKT_LIMIT] = { 0 };  /* RTM vpd packets */
...
BOOL         rtmIsPresent = FALSE;                      /* RTM presence */
...
STATUS sysMotVpdHdrVld
    ...
    if (((*pEyecatcher    != 0x4d4f544f)  ||  /* MOTO */
        ...
        ((*(pEyecatcher)  != 0x454d4552)  ||  /* EMER */
        (*(pEyecatcher+1) != 0x534f4e20)))    /* SON  */
        return (ERROR);
...
/******************************************************************************
*
* sysMotRtmVpdPktInit - initialize a vital product data structure for RTM.
*
* This routine reads the vital product data header from the RTM serial eeprom 
* if present and validates it. If the header is valid, the remainder of the 
* vpd data is read from the serial eeprom and parsed into vpd packets for 
* general purpose use. 
*
* RETURNS: OK, if successful or ERROR if unsuccessful or RTM not present.
*/

STATUS sysMotRtmVpdPktInit
    (
    UCHAR         devOffset, /* offset to vpd within the serial eeprom */
    VPD *         pVpd,      /* address of vpd structure */
    VPD_PACKET ** pVpdPtr,   /* address of packet ptr array */
    ...
    *pVpdPtr = NULL;
    ...
    if (RTM_VPD_READ ((UINT32)devOffset, (UCHAR *)pVpd, 
                       sizeof(VPD_HEADER)) == OK)
        {
        ...
        if (sysMotVpdHdrVld (pVpd) == OK)
            ...
            if (RTM_VPD_READ ((UINT32)(devOffset + sizeof(VPD_HEADER)),
                              (UCHAR *)&pVpd->packets, 
                              (UINT32)(pVpd->header.size - 
                              sizeof(VPD_HEADER))) == OK)
               {

               /* Parse the raw vpd data into vpd packets. */

               status = sysMotVpdPktParse (pVpd, pVpdPtr, PktLimit);
               }
            }
        else
            {
...
STATUS sysMotVpdInit (void) 
    ...
    if ((sysMotVpdPktGet (VPD_PID_PID, 0, &sysVpdPkts[0], &pVpdPkt) == OK) &&
        ...
        for (idx = 0; idx < ((pVpdPkt->size)-7); idx++)
    ...
    /* Attempt to read RTM VPD. If successful, RTM is present */

    rtmIsPresent = (sysMotRtmVpdPktInit (VPD_BRD_OFFSET, &sysRtmVpd,
                                         &sysRtmVpdPkts[0],
                                         VPD_PKT_LIMIT) == OK);
    return (OK);
    }
