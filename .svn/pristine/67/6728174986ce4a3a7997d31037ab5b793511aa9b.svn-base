...
--------------------
01c,25nov10,swu  fix sysFlashWrite() can not re-flash the last sector.
                 (WIND00241089)
01b,14jul09,my_  made 1st param of sysSectorErase() volatile,
                 use (FLASH_DEF *) to cast 1st param passed to sysFlashWrite()
                 (WIND173188)
01a,20oct08,b_m  derived from version 01c of fsl_ads5121e/flashMem.c
*/

/*
DESCRIPTION
This library contains routines to manipulate flash memory.
Read and write routines are included.

The macro values FLASH_ADRS, FLASH_SIZE, and FLASH_WIDTH must be defined to
indicate the address, size (in bytes), and the data-access width (in bytes)
of the flash memory.
...
amount of flash that is writable, so why bother.  What it was really intended
to address was flash that is only block writable, i.e. you can only write a
complete block at a time.  To properly handle block memory, you must copy
the old block of memory to a buffer, update the part of the buffer that is
to be changed, and then write back the complete buffer in a single write
operation.

The accesses to non-volatile memory, and flash control registers needs to be
abstracted.  Macros should be used for all actual I/O operations.
*/
...
* The timing loop can be adjusted on a board-dependent basis by defining values
...
* SYS_FLASH_DELAY_INCR
* .LP
* The values SYS_FLASH_DELAY_SHIFT and SYS_FLASH_DELAY_ADJ convert microseconds
* into a board-dependent tick-count.  This routine can call a user-defined hook,
* sysFlashBoardDelay(), which creates a delay for a number of board-dependent
* ticks as specified by SYS_FLASH_DELAY_INCR.  To use sysFlashBoardDelay(),
* define SYS_FLASH_BOARD_DELAY in config.h.
*
* RETURNS: N/A
*
...
*
* This routine polls a specified address on a 29F\f2xxx\f1 flash device until
* the device operation at that location completes or times out.
*
...
* <pFB>, if not NULL.  If NULL, data is taken as a repeated sequence of <value>.
* The parameter <offset> must be appropriately aligned for the width of the
* Flash devices in use.
*
...
STATUS sysFlashWrite
    ...
    int loopCount = size / FLASH_WIDTH;
    STATUS retVal = OK;
...
*
* This routine copies the contents of flash memory into a specified string.
...
* use a semaphore to ensure mutually exclusive access to flash memory.
*/
...
STATUS sysFlashSet
    ...
    if (sysSectorErase (FLASH_CAST(FLASH_SECTOR(FLASH_ADRS + offset))) == ERROR)
        {
        free (tempBuffer);
        return (ERROR);
        }
