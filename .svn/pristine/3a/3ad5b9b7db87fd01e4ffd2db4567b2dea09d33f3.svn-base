...
#define INCLUDE_VXIPI
    /*
      Include in the vxWorks image only.
...
/* Configuring AMP is tricky when sharing resources so try to prevent 
 * the two cores from trying to use or interoperate with the same devices
 */
...
#define TM_ANCHOR_ADRS (0x4400 + 0x20000000)
#endif
 ...
 * loaded using WRLOAD so undefine the falsh utility.
 */
...
#if defined (AMP_CORE1) && defined(LEGACY_AMP)
#define ROM_TEXT_ADRS 0xFFC00100
#define ROM_BASE_ADRS 0xFFC00000
...
#ifdef LEGACY_AMP
#define AMP_CORE1_TEXT_ADRS     0xFFC00100
#else
