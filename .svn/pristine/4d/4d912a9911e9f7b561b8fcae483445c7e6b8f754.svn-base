...
--------------------
01j,07jun12,mpc  Fixed potential memory corruption caused by sysNvRamGet().
                 (WIND00101057)
01i,24feb09,x_z  Remove gnu compiler warning.(WIND00098576)
...
STATUS sysMacOffsetGet
	...
	*pOffset = ifUnit * (MAC_ADRS_LEN); /* Round up to 32 bit word */
...
#else
    memcpy(pAddr,macAddr[unit],MAC_ADRS_LEN);
    return(OK);
#endif
    }

#else /* defined(INCLUDE_ETSEC_VXB_END) || defined(INCLUDE_ETSEC_VXB_END2) */
...
#else /* defined(INCLUDE_TSEC_VXB_END) || defined(INCLUDE_TSEC_VXB_END2) */
/***********************************************************************
*
