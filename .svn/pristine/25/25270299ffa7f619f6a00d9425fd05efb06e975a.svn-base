/* romInit.s - Freescale MPC830x board ROM initialization module */
...
--------------------
01b,12oct11,e_d  set HID2 to correct value. (WIND00307278)
01a,30jun11,e_d  derived from version 01a of fsl_mpc8308_rdb/romInit.s
*/
...
cold:
    ...
    /*
     * Set HID2[10] [13] [14] bit to 1,  SPCR[14] to 1 and ACR[10:11] to 01/10,
...
start:
    ...
    WRITEADR(r6, r7, ELBC_OR0, (OR_SIZE_8M | 0x0f47))
    ...
    WRITEADR(r6, r7, ELBC_OR1, (0xFFFF8796))
#else

    /* WINDOW 0 - NOR FLASH */

    WRITEADR(r6, r7, ELBC_BR1, (0xfe000000 | 0x1001))
    WRITEADR(r6, r7, ELBC_OR1, (OR_SIZE_8M | 0x6ff7))

    WRITEADR(r6, r7, LBLAWBARn(1), 0xfe000000)
    WRITEADR(r6, r7, LBLAWARn(1), (LAWAR_ENABLE | LAWAR_SIZE_8MB))

    /* WINDOW 1 - NAND FLASH */

    WRITEADR(r6, r7, LBLAWBARn(0), NAND_BUFFER_BASE)
    WRITEADR(r6, r7, LBLAWARn(0), (LAWAR_ENABLE | LAWAR_SIZE_32KB))

    WRITEADR(r6, r7, ELBC_BR0, (NAND_BUFFER_BASE | 0x0C21))
    WRITEADR(r6, r7, ELBC_OR0, (0xFFFF8796))
#endif /* BOOT_FROM_NAND */
...
tlb_write_loop:
...
#else
    WRITEADR(r6, r7, DDRLAWARn(0), (LAWAR_ENABLE | LAWAR_SIZE_256MB))
#endif /* MPC8309_BOARD */
    ...
    WRITEOFFSET(r6, r7, (CS0_CONFIG), 0x80844102)

    WRITEOFFSET(r6, r7, (TIMING_CFG_0), 0x40220802)
    WRITEOFFSET(r6, r7, (TIMING_CFG_1), 0x3935D322)
    WRITEOFFSET(r6, r7, (TIMING_CFG_2), 0x1F9048CA)

    WRITEOFFSET(r6, r7, (DDR_SDRAM_CFG_2), 0x60401000)
    WRITEOFFSET(r6, r7, (DDR_SDRAM_MODE_CFG), 0x440C0232)
...
delayddr1:
...
#ifndef MPC8309_BOARD
    WRITEOFFSET(r6, r7, DDR_SDRAM_CFG, 0xC3100000)
    sync
#else
    ...
    WRITEADR(r6, r7, M83XX_OCCR, 0xc000c040)
    isync
