...
--------------------
01d,24jul07,dtr  Fix counter mask WIND00093512 removed modulo as inefficent.
...
LOCAL UINT32 nextIntLine=0; /* point to the next interrupt line to check */
...
LOCAL UINT32 tsi108IntSenses[] =
    ...
    (TSI108_IRQ_SENSE_EDGE  | TSI108_IRQ_ACTIVE_HIGH),  /* UART0 */ 
    (TSI108_IRQ_SENSE_EDGE  | TSI108_IRQ_ACTIVE_HIGH),  /* UART1 */ 
...
LOCAL int mpicGetPciSource(void)
	...
	for (i = 0; i < 4; i++,nextIntLine++)
	    {
	    if (temp & (1 << (nextIntLine & 0x3)))
		{
		irq = IRQ_PCI_INTA + (nextIntLine & 0x3);
		nextIntLine++;
		break;
		}
	    }
