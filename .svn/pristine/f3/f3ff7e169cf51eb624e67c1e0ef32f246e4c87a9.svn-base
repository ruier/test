...
/*
 * Copyright (c) 1995, 1997, 2001-2002, 2004-2007 Wind River Systems, Inc.
 *
...
01q,31mar06,dtr  Modifications for Tsi108 UART: 1.Divisor calculations 
                 in ns16550BaudSet() routine; 2.Workaround for UART errata U2
...
#define TSI108_ERRATA_U2 /* Use workarounds for Tsi108 Errata U2 */
...
#ifdef TSI108_ERRATA_U2	/* Workaround for Tsi108 UART errata U2 */
    /*
     * It is safe (non distructive) to use LSR istead of IIR to identfy
     * an interrupt source.
     */
    intStatus = REG(LSR, pChan); /* read LSR to get interrupt source(s) */

    if (intStatus & LSR_DR)
	ns16550IntRd (pChan);  	/* RxChar Avail */

    if (intStatus & LSR_THRE)
	{
        char outChar;
...
#ifdef TSI108_ERRATA_U2
		/*
		 * Workaround for Tsi108 UART errata U2
		 * Tx interrupts are "lost" after reset.
		 * SW have to kick-start Tx interrupts by sending some data
		 * in non-interrupt mode. Code below assumes that it is safe
		 * to send the firt byte without any status checking.
		 */
		if (first_byte != 0)
		    {
		    char outChar;
