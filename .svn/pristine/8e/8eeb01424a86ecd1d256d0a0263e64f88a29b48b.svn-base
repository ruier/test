...
/*
 * Copyright (c) 2002-2004, 2008, 2011, 2012 Wind River Systems, Inc.
 *
...
--------------------
01p,21aug12,l_z  Better implementation for sysPCGet. (WIND00076160)
01o,19dec11,agf  Moved L1 parity handling to arch lib. (WIND00309426)
01n,30oct08,x_z  remove all codes related to BUCSR(spr1013) and saved starttype
                 in sysInit.(WIND00127193)
01m,30jul04,dtr  adding func sysTimeBaseLGet. 
	         Modify instruction parity handler to reboot if errata#30 hit.
01l,22jul04,dtr  Add sysL1CsrSet routine.
01k,05jun04,dtr  Enabling branch prediction and L1 parity error handler.
01n,16aug04,dtr  Merge from T2.2.1.
01m,10jun04,dtr  Condiitional include of L1 Erra6ta Fix.
01l,21may04,dtr  Fix complier error for gnu.SPR 97609
01k,01apr04,dtr  Enabling branch prediction.
01j,20nov03,dtr  Flush cache using load and dcbf so can move 
                 cachePpcReadOrigin to any cacheable memory. 
...
FUNC_BEGIN(sysInit)
	...
	xor   r0, r0, r0
        ...
        addi  r3, r0, 0x1000
        mtspr IVOR15, r3
        addi  r3, r0, 0x1100       /* assigning 0x1100 for SPU */
        mtspr IVOR32, r3
        addi  r3, r0, 0x1200       /* assigning 0x1200 for FP data */
        mtspr IVOR33, r3
        addi  r3, r0, 0x1300       /* assigning 0x1300 for FP round */
        mtspr IVOR34, r3
        addi  r3, r0, 0x1400       /* assigning 0x1400 for perf mon */
        mtspr IVOR35, r3
	...
	mtspr PID2 , r0
        ...
        /*
         * TLB1 #0.  RAM - Cached 0x00000000 -> 0x000000000.  256MB
         * Attributes: SX/SW/SR
         */
        ...
        ori    r5,r5,0x0900          /* TS = 0, TSIZE = 256 MByte page size*/
        mtspr  MAS1, r5
        addis  r6,0,0xf000           /* EPN = 0xfff00000*/
        ori    r6,r6,0x000a          /* WIMGE = 01000*/
        mtspr  MAS2, r6
        addis  r7,0,0xf000           /* RPN = 0xfff00000*/
	...
	/* Initialise the Local Address Windows */
        lis     r7, HIADJ(FLASH_BASE_ADRS >> LAWBAR_ADRS_SHIFT)
        addi    r7, r7, LO(FLASH_BASE_ADRS >> LAWBAR_ADRS_SHIFT)
	stw     r7, 0x0(r6)
