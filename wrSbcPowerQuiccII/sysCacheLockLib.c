...
/*
 * Copyright (c) 2000-2001, 2004-2005, 2010 Wind River Systems, Inc.
 *
...
--------------------
01i,12apr10,jpb  Added logLib.h for _func_logMsg.
01h,02sep05,mil  Added use of INCLUDE_MEM_ALLOT marco.
01g,08oct04,mil  Changed cachePpcEnable/Disable to use API. (SPR#102422)
01f,22sep04,mdo  Documentation fixes for apigen
...
caches (harward-style) for the L1. The lock routine locks the entire
cache after loading it with specified code or data region. When the cache is
locked, data/instruction access to regions not locked in the cache do not
hit the cache.

L1 locking routines can be used to lock only one region of memory. Multiple
calls to lock a region results in unlocking the previous region. The
...
/* exclude cacheDmaMalloc/Free if malloc/free is not available */
...
* 32-byte boundary.
*
* Multiple calls to this routine silently unlocks the specified cache to
* lock the new region.
*
...
STATUS sysCacheLock
    ...
    if (!ALIGNED(adrs, 32))
        ...
        if (_func_logMsg != NULL)
            _func_logMsg("\n Memory Address not aligned on 32 byte boundary\n",
                         1,2,3,4,5,6);
        return (ERROR);
        }
...
#endif /* INCLUDE_CACHE_SUPPORT && !INCLUDE_MEM_ALLOT */
