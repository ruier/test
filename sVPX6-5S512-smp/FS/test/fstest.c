...
STATUS fsFdtReadTest(UINT32 vuiDevId,UINT32 vuiClustId,UINT32 vuiOffset)
{
    ...
    pstVol = &gstFsCtrl.astVol[0];
    pstFDT = cacheDmaMalloc(512);
    bzero((CHAR*)pstFDT,sizeof(FS_FDT_S));
    
    if(OK != gstFsCtrl.stFDTFunc.fsReadFDT(pstVol,pstFDT,vuiClustId,vuiOffset))
    {
        FS_LOG(FS_DBG_ERR,"read FDT failed\n",vuiDevId,0,0,0,0,0);
        return ERROR;
    }
    FS_LOG(FS_DBG_INFO,"read stat 0x%x offset 0x%x size 0x%x attr %x\n",
              pstFDT->ucFileStat,vuiOffset,(UINT32)(pstFDT->ullFileSize.lo),
              pstFDT->ucAttr,0,0);
    return OK;
}
...
STATUS fsPerfTest(CHAR *vpcOutName,UINT32 uiAddr,
			UINT32 vuiOper,UINT32 vuiTestTime, UINT32    uiByteNum )
{
    INT32     iFd = 0;
    UINT8    *pucBuf = NULL;
    /*UINT32    uiByteNum = 0;*/
	...
	if(2 == vuiOper)
	{
		iFd = open(vpcOutName,0x209,0);
	}
	else
	{
		iFd = open(vpcOutName,0,0);
	}
    if(iFd < 0)
    {
        FS_LOG(FS_DBG_ERR,"open file failed\n",0,0,0,0,0,0);
    ...
    tickStart = tickGet();
    while(uiTime < vuiTestTime)
    {
		if(2 == vuiOper)
		{
			uiWrLen = write(iFd,pucBuf,uiByteNum);
			uiLen = uiLen + uiWrLen /*(uiWrLen / 0x100000)*/;
            if(0 == uiWrLen)
            {
        ...
        {
			uiRdLen = read(iFd,pucBuf,uiByteNum);
			if(uiRdLen != uiByteNum)
			{
				printf("read file size %d %d\n",uiRdLen,uiByteNum);
			}
			uiLen = uiLen + uiRdLen/*(uiRdLen / 0x100000)*/;
            if(uiRdLen <= 0)
            {
				taskDelay(60);
                printf("read file size 0 read later\n",uiLoop);
            }
		}
        ...
        if((tickEnd-tickStart) > 60)
        {
			...
			logMsg("--cpu %d tick dma oper %d speed %dMB/s %d tick ok\n",cpus,
                    vuiOper,(tickSpeed),(tickEnd-tickStart),0,0);
...
STATUS multiTaskTest( UINT32    uiByteNum ,UINT32 uiCnt)
{
	INT32      iTid = 0;
	cpuset_t   affinity;

	iTid = taskCreate ("WRITE", 100, 0,0x800000, fsPerfTest,"/vol/test.bin",
		               0x10000000,2,uiCnt, uiByteNum , 0, 0, 0, 0, 0);
    if (iTid == NULL)
    {
        return (ERROR);
    }
...
#if 1	
	taskDelay(600);
	iTid = taskCreate ("READ", 100, 0,0x800000, fsPerfTest,"/vol/test.bin",
		               0x18000000,1, uiCnt, uiByteNum , 0, 0, 0, 0, 0);
    if (iTid == NULL)
    {
        return (ERROR);
    }
...
STATUS fsPerfTest1(CHAR *vpcOutName,UINT32 vuiOper)
{
    ...
    {
        fsWrite(pstFd,pucBuf,uiByteNum);
    }
...
void fsPreAllocTest()
{
    ...
    int len = 0x600000;
    int *wrBuf = NULL;
    UINT64 offset = 0x100000ULL;
	UINT32 uiHi = 0;
	UINT32 uiLo = 0;

	uiHi = (UINT32)(offset >> 32);
	uiLo = (UINT32) offset;
	FS_LOG(FS_DBG_ERR,"set file size to %x.%x \n",uiHi,offset,0,0,0,0);
	
    fd = open("/vol/test.bin",O_WRONLY|O_CREAT,0);
    ioctl(fd,FIOTRUNC,&offset);
    close(fd);
}
