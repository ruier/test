...
extern VOID ahciSendIdentify(AHCI_DRV_S *pstDrv);


UINT32 guiAhciDbgLevel = AHCI_DBG_INFO;
...
LOCAL PCI_DEVVEND vxbAhciIdList[] =
{
    ...
    {NVME_DEVICE_SAM_ID,NVME_VENDOR_SAM_ID}
};

/*LOCAL UINT32 ahciPhyCfg[AHCI_MAX_PHY] = {7,8,9,10,11,12,0xFF,0xFF};*/
LOCAL UINT32 ahciPhyCfg[AHCI_MAX_PHY] = {3,4,5,7,8,0xFF};/*phy 号，盘的位置*/
UINT32		uiPhyNumEx = 0;
...
VOID ahciStartFISRx(AHCI_DRV_S *pstDrv)
{
    ...
    AHCI_LOG(AHCI_DBG_INFO,"phy %d start FIS status 0x%x\n",pstDrv->uiPhyId,
             uiVal,0,0,0,0);
}
...
STATUS ahciEnableInt(AHCI_DRV_S *pstDrv)
{
	...
	AHCI_LOG(AHCI_DBG_INFO,"phy %d port enable 0x%x\n",pstDrv->uiPhyId,
             uiVal,0,0,0,0);
...
STATUS ahciDrvInit(UINT32 uiPhyNum)
{
	...
	AHCI_LOG(AHCI_DBG_ERR,"phy %d ahci drv 0x%x init ok\n",
	         pstDrv->uiPhyId,(UINT32)pstDrv,0,0,0,0);
...
LOCAL void vxbAhciInstInit2(VXB_DEVICE_ID pDev)
{
	...
	AHCI_LOG(AHCI_DBG_INFO,"phy %d ahci register addr 0x%x\n",uiPhy,
...
void ahciDumpReq(AHCI_DRV_S *pstDrv)
{
    ...
    for(uiLoop = 0;uiLoop < AHCI_MAX_REQ;uiLoop++)
    {
        ...
        {
			logMsg("req %d %p req num %d\n",uiLoop,&pstDrv->astReq[uiLoop],
				   pstDrv->uiReqNum,0,0,0);
        }
    }
}
