...
STATUS sysIpChange(char *string, unsigned int netNum)
{
	...
	if(OK != flashProgram(FLASH_BASE_ADRS1,FLASH_BOOT_OFFSET,uiBuf,FLASH_SECTOR_BYTE,0))
	{
		free(pucBuf);
		free(uiBuf);
...
/*******************************************************************************
* function:     sysNetIpGet;  从flash中参数的存放位置，提取IP地址              *
* description:  get start IP of socket server ;                                *
* parameter:                                                                   *
*               paramString: sysBootLine;                                      *
*               ipnum;                                                         *
* return:       if success return ok;                                          *
*******************************************************************************/
INT32 sysNetIpGet(char *ipbuf, int netNum)
{
	UINT8    *pucBuf = NULL;
	UINT32    flag = 0;
	UINT32    uiflag = 0;
	UINT8     uiLoop = 0;
	UINT8     uiOffset = 0;
	UINT8      netLen = 16;/*IP地址所占地址空间不超过16Byte*/

	pucBuf = malloc(FLASH_SECTOR_BYTE);/*分配一个扇区的BUF*/
	if(NULL == pucBuf)
	{
	    printf("malloc memory failed\n");
	    return ERROR;
	...
	/*将flash中启动参数复制到BUF中进行提取*/
	bcopyBytes (FLASH_BOOT_OFFSET,pucBuf,FLASH_SECTOR_BYTE);

	for(flag = 0; flag < FLASH_SECTOR_BYTE; flag++)
	{
	    if((pucBuf[uiOffset + flag] == 'e')&&(pucBuf[uiOffset + flag + 1] == '='))
	    {
	        break;
	    }
	}
	...
	if(flag >= FLASH_SECTOR_BYTE)
	{
	    printf("read boot parm error when get ip addr!\n");
    	    free(pucBuf);
	    return ERROR;
	}
	...
	if (netNum == 0)
	{
	    memcpy(ipbuf, &pucBuf[uiOffset + flag + 2], 16);/*将启动IP复制到BUF中*/
		
		for( flag = 0; flag < 16; flag++)/*IP地址的长度最长为15*/
		{
			if(ipbuf[ flag ] == ':')
			{
				for(uiLoop = flag; uiLoop < 16; uiLoop++)
				{
					ipbuf[ uiLoop ] = '\0';/*多余部分添'\0'*/
				}	
				break;
			}
		}
	...
	{
		memcpy(ipbuf, &pucBuf[uiOffset + NV_IP_ADRS_OFFSET], netLen);		
	}
	else if (netNum == 2)
	{
		memcpy(ipbuf, &pucBuf[uiOffset + NV_IP_ADRS_OFFSET + netLen], netLen);		
	}
	else
	{
		memcpy(ipbuf, &pucBuf[uiOffset + NV_IP_ADRS_OFFSET + netLen*2], netLen);		
	}
...
VOID extPabackIntInit(VOID)
{
    ...
    intOpenInit(2,EPIC_MODE_LOW_LEV,pabackintISR,NULL);
    intMaskEnable(2);
}
#endif
#if 1
