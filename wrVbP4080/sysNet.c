...
01f,07jun12,mpc  Fixed potential memory corruption caused by sysNvRamGet().
                 (WIND00101057)
01e,05sep09,wap  Fix M command in bootrom
01d,28aug09,wap  Add support for DTSEC ports
...
LOCAL UINT8 glbEnetAddr[MAX_MAC_ADRS][MAC_ADRS_LEN + 1] = {
    ...
    { WR_ENET0, WR_ENET1, WR_ENET2, CUST_ENET3_6, CUST_ENET4, CUST_ENET5, 0},
    { WR_ENET0, WR_ENET1, WR_ENET2, CUST_ENET3_7, CUST_ENET4, CUST_ENET5, 0},
    { WR_ENET0, WR_ENET1, WR_ENET2, CUST_ENET3_8, CUST_ENET4, CUST_ENET5, 0},
    { WR_ENET0, WR_ENET1, WR_ENET2, CUST_ENET3_9, CUST_ENET4, CUST_ENET5, 0}
};
...
STATUS sysMacOffsetGet
	...
	/* Round up to 32 bit word */

	*pOffset = ifUnit * ROUNDUP(MAC_ADRS_LEN, sizeof(int)); 
        *ppEnet  = (char*)glbEnetAddr[ifUnit];
	}
    else if (strcmp (ifName, "dtsec") == 0)
        {
        if (ifUnit >= MAC_OFFSET_TGEC)
            return (ERROR);
	...
	*pOffset = ifUnit * ROUNDUP(MAC_ADRS_LEN, sizeof(int));
    ...
    else if (strcmp (ifName, "tgec") == 0)
        {
        if (ifUnit > 1)
            return (ERROR);

	*pOffset = (ifUnit + MAC_OFFSET_TGEC) * 
	       ROUNDUP(MAC_ADRS_LEN, sizeof(int)); /* Round up to 32 bit word */
        *ppEnet  = (char*)glbEnetAddr[(ifUnit + MAC_OFFSET_TGEC)];
        }
    else
    	return(ERROR);
...
int sysMacIndex2Unit
    ...
    {
    /* Indices 8 and 9 correspond to tgec0 and tgec1 */
    if (index == MAC_OFFSET_TGEC)
        return (0);
    if (index == MAC_OFFSET_TGEC + 1)
        return (1);
