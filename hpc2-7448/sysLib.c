/* sysLib.c - Freescale HPC II system-dependent library */
...
01k,15jul09,my_  fix diab compiler warnings (WIND00173188)
01j,22apr08,z_l  Add vxBus system clock support
01i,20sep07,pmr  fix apigen error
01h,08aug07,agf  remove duplicate variable definitions
01g,31mar06,dtr  Cleanup.
01f,21mar06,dtr  Add memory coherency for local memory by default.
01e,07feb06,avb  added sysUsdelay() routine, modified sysMsDelay() 
01d,01feb06,dtr  tidyup
01c,06dec05,avb  clean up to meet vxWorks coding style requirements
01b,18aug05,vsc  added flash programming functions
...
#if defined (INCLUDE_SYS_COPY_TO_FLASH)
#include <stat.h>
#endif
...
#define SYS_MODEL   "Freescale HPC-II Platform "
...
LOCAL UINT32 extIntSenses[] =
    {
    (TSI108_IRQ_SENSE_LEVEL | TSI108_IRQ_ACTIVE_LOW),  /* INT[0] XINT0 FPGA */
    (TSI108_IRQ_SENSE_LEVEL | TSI108_IRQ_ACTIVE_LOW),  /* INT[1] XINT1 FPGA */
    (TSI108_IRQ_SENSE_LEVEL | TSI108_IRQ_ACTIVE_LOW),  /* INT[2] PHY_INT */
    (TSI108_IRQ_SENSE_LEVEL | TSI108_IRQ_ACTIVE_LOW)   /* INT[3] RESERVED */
    };
...
/* _pSysBatInitFunc needs to be set to the above to make use of
 * the additional BAT's  on the MPC 74x5.
...
*
* This is a modification of sysUpdateFlash to support partial flash
* updates versus erasing and writing to the entire flash.
*
* ( /Tornado2.2/target/config/sp7447/sp7447/sysUpdateFlash)
*
...
* routines (from src/drv/mem/flashMem.c).  It will copy data from 
* "filename" to the flash device.  It writes data to the flash device 
* via sysFlashWrite(), and it will verify that the data was written 
* correctly.  Writes to flash are wrapped within taskLock() and intLock().
*
* If Target Server File System is enabled and have set the root directory as:
*
*    C:\Tornado2.2\target\proj\HPC2_Boot\default
*
* and there is a vxWorks.bin then:
*
*    sysCopyFileToFlash("/tgtsvr/vxWorks.bin", 0x0)
*
* will copy the vxWorks.bin image to FLASH_ADRS + 0.
*
* On success, this message will be printed.:
*   The flash has been updated without error.
* ->
*
* Note: bootrom, bootrom_uncmp, vxWorks_rom, vxWorks_rom_cmp, 
*  and vxWorks.st_rom are the tested and supported images.
*
...
STATUS sysCopyFileToFlash		
    ...
    /* check if the file can be open'd. */
...
* routines (from src/drv/mem/flashMem.c).  It will copy data from 
* <pMemSrc> of <srcSize> to the flash device.  It writes data to the flash 
* device via sysFlashWrite(), and it will verify that the data was written 
* correctly.  Writes to flash are wrapped within taskLock() and intLock().
*
...
#if defined INCLUDE_PCI 	/* board level PCI routines */
...
/*******************************************************************************
*
* sysClkFreqGet - get processor bus clock
*
* This routine returns the value of sysPbClock, which is calculated in 
* sysGetPbClock()
*
