...
--------------------
01c,06nov08,y_w  Code clean up.
01b,25apr08,mdg  Compiler warnings.
...
.CS
 programRemoteBootImage
    ...
    syntax: programRemoteBootImage "-n<fname>","-d<devname>","-b<boot block","-h<host>","-g<gateway>"
.CE
...
#include "wrn/coreip/net/utils/routeCmd.h"
...
LOCAL STATUS flashProgram
    ...
    /*
     * bootBlockIsA = TRUE  if Boot Block A is selected.
     * bootBlockIsA = FALSE if Boot Block B is selected.
     */
...
STATUS programRemoteBootImage
    ...
    if (hostIP != NULL)
       {
       int hostnet;
       char hostnetString[20];
       char routeCmd[80];
       struct in_addr hostaddr;

       hostaddr.s_addr = inet_addr(hostIP);
       hostnet = inet_netof(hostaddr);

       hostaddr.s_addr = hostnet << 8;
       inet_ntoa_b(hostaddr, hostnetString);

       sprintf(routeCmd,"add -host %s %s",hostnetString,params.gad);

       /* add a route to this network */

       routec(routeCmd);

       printf("Route Add %s via %s\r\n", hostnetString, params.gad);
       }
