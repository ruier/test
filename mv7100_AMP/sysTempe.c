...
/*
 * Copyright (c) 2008-2009, 2013 Wind River Systems, Inc.
 *
...
--------------------
01g,26jun13,d_l  Fix coverity errors. (WIND00423743)
01f,10feb09,e_d  Move vmeOutWinCfg for diab compiler.
01e,08dec08,y_w  Modify to let master (SCON) and slave use the same images
01d,17mar08,mdg  Updated for the mv7100.
...
LOCAL spinlockIsr_t sysTempeSpinlock;
...
LOCAL UINT32 tempeResetTbl [] =
    ...
    NOT_INIT_TEMPE_REG (TEMPE_MXLA_MNGN_INTP_INTL, autoConfiged)
...
extern void sysUserLedSet (UINT8 whichLed, int value);
...
void sysTempeInit (void)
    ...
    LOCAL VME_IN_WINDOW_CFG vmeInWinCfg_scon [] =
                {
                    { VME_IN0_CFG_PARAMS },
                    { VME_IN1_CFG_PARAMS },
                    { VME_IN2_CFG_PARAMS },
    ...
    LOCAL VME_IN_WINDOW_CFG vmeInWinCfg_slv [] =
                {
                    { VME_INx_CFG_DISABLED },
                    { VME_IN1_CFG_PARAMS },
                    { VME_IN2_CFG_PARAMS },
                    { VME_IN3_CFG_PARAMS },
                    { VME_IN4_CFG_PARAMS },
    ...
    /*
     * Check SCONS bit (bit-8) of VMEBus Status RegisterUse in TSI148 to
     * know if this target is a VME system controller, and use USR_LED1 to
     * show the status:
     *
     * Orange: SCON Enable
     * Yellow: SCON Disable
    ...
    vmebus_stat = TEMPE_READ32(TEMPE_REG_BASE,TEMPE_VSTAT);

    if ((vmebus_stat & 0x100) == 0) /* bit8 = 0, SCON disable */
        {
        /* turn on LED1, color = YELLOW */

        sysUserLedSet (1, 2);

        vInWinCount = sizeof (vmeInWinCfg_slv) / sizeof (VME_IN_WINDOW_CFG);
        }
    else                            /* bit8 = 1, SCON enable */
        {
        /* turn on LED1, color = ORANGE */

        sysUserLedSet (1, 3);

        vInWinCount = sizeof (vmeInWinCfg_scon) / sizeof (VME_IN_WINDOW_CFG);
        }

    vOutWinCount = sizeof (vmeOutWinCfg) / sizeof (VME_OUT_WINDOW_CFG);

    spinLockIsrInit(&sysTempeSpinlock, 0);
   ...
   for (i = 0; i < vOutWinCount; i++)
    ...
    for (i = 0; i < vInWinCount; i++)
        ...
        if ((vmebus_stat & 0x100) == 0) /* bit8 = 0, SCON disable */
            {
            sysTempeInMapInit (i, &vmeInWinCfg_slv [i]);
            }
        else
            {
            sysTempeInMapInit (i, &vmeInWinCfg_scon [i]);
            }
        }
...
BOOL sysVmeBusLock (void)
    ...
    ticksToWait = LOCK_TIMEOUT *
                  ((sysClkFreqGet()/DECREMENTER_PRESCALE)/USECS_PER_SECOND);
