...
/*
 * Copyright (c) 2004, 2005, 2008, 2011 Wind River Systems, Inc.
 *
...
--------------------
01d,27jan11,e_d Modified vxIpiEmit to vxIpiPhysEmit function.(WIND00252647)
01c,17mar08,mdg Updated for mv7100.
...
IMPORT STATUS vxIpiPhysEmit (INT32 ipiIntrptId, phys_cpuset_t phys_cpuset);
...
void sysTempeIntInit (void)
    ...
    pciConfigInByte (TEMPE_PCI_BUS,TEMPE_PCI_DEV,TEMPE_PCI_FCN,
                     PCI_CFG_DEV_INT_LINE, &intLevel);
#endif
    (void) intConnect (INUM_TO_IVEC((UINT32)intLevel), sysTempeVmeIntr, 0);
...
STATUS sysIntEnable
    ...
    if (intLevel < 1 || intLevel > 7)
        ...
        if (_vxb_intEnableRtn == NULL)
            return(ERROR);
        return (_vxb_intEnableRtn(intLevel));
    	}
...
STATUS sysIntDisable
    ...
    if (intLevel < 1 || intLevel > 7)
        ...
        if (_vxb_intDisableRtn == NULL)
            return(ERROR);
        return (_vxb_intDisableRtn(intLevel));
    	}
