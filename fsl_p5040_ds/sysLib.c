/* sysLib.c - Freescale P5040DS board system-dependent library */
...
--------------------
01a,04sep12,d_l  adapted from fsl_p5020_ds version 01n.
*/

/*
DESCRIPTION
This library provides board-specific routines for p5040ds. The chip drivers
included are:
...
* Status registers, internal Memory Map, and local FLASH RAM are set here.
*
...
LOCAL void sysMuxInit(void);
LOCAL void sysMux1Ctrl
    (
    UINT32  dtsecFmanUnit,
    UINT32  dtsecNum
    );
LOCAL void sysMux2Ctrl 
    (
    UINT32 fmanUnit
    );
...
#define SYS_MODEL_P5021     "Freescale QorIQ P5021"
#define SYS_MODEL_P5021E    "Freescale QorIQ P5021E"
#define SYS_MODEL_P5040     "Freescale QorIQ P5040"
#define SYS_MODEL_P5040E    "Freescale QorIQ P5040E"
...
LOCAL struct vxbDeviceMethod p5PlbMethods[] =
    {
#ifdef INCLUDE_AMP
...
/*******************************************************************************
*
* sysMux1Ctrl - Control MII1 MDIO pins mux switch 
*
        ...
        switch (srds)
            ...
            case 6:
                if (dtsecFmanUnit == 0)
                    ...
                    brdCfg1 |= (MII1_SLOT_3 << PX_CFG1_EMI1_SEL_SHIFT);
                break;
            case 1:
            case 7:
                brdCfg1 |= (MII1_SLOT_3 << PX_CFG1_EMI1_SEL_SHIFT);
...
char * sysModel (void)
    ...
    device = *P5_SVR(CCSBAR);

    switch(device & 0xffff0000)
        {
        case 0x820C0000:
            retChar = SYS_MODEL_P5040E;
            break;
        case 0x820D0000:
            retChar = SYS_MODEL_P5021E;
            break;
        case 0x82200000:
            retChar = SYS_MODEL_P5040;
            break;
        case 0x82050000:
            retChar = SYS_MODEL_P5021;
            break;
        default:
...
UINT32 sysCpuNumGet (void)
    {
    return (1 + ((*P5_EPICFRR(CCSBAR) >> 8) & 0x1f));
    }
...
UINT32 sysClkFreqGet (void)
    ...
    platRatio = P5_SYS_PLL_RAT(CCSBAR);
...
#   endif /* INCLUDE_SHOW_ROUTINES */
    ...
    ddrRatio = P5_MEM_PLL_RAT(CCSBAR);
...
#   endif /* INCLUDE_SHOW_ROUTINES */
    ...
    for (n = 0; n < sysCpuNumGet(); n++)
        {
        ccRatio[n] = P5_CCN_PLL_RAT(CCSBAR, n);
...
#   ifdef INCLUDE_SHOW_ROUTINES
    ...
    for (n = 0; n < sysCpuNumGet(); n++)
        {
        UINT32 ccPllSel = P5_CN_PLL_SEL(CCSBAR, n);
...
*
* This routine initializes various features of the P5040DS board. It sets up
...
void sysHwInit (void)
     ...
     *
     * In Rev 1.0 silicon, to avoid the behavior Freescale recommends the
...
/* allow only the CPU0 image to initialize the L3 cache */
...
#if !defined(INCLUDE_AMP) || defined(INCLUDE_AMP_CPU_00)
#if (defined(INCLUDE_L3_CACHE) && defined(INCLUDE_CACHE_SUPPORT))
    sysL3CacheInit();
#endif /* INCLUDE_L3_CACHE  && INCLUDE_CACHE_SUPPORT*/
#endif /* !INCLUDE_AMP || INCLUDE_AMP_CPU_00 */
    ...
    sysClkFreqGet();
        ...
        {
        UINT32 chipRev;
        chipRev = (vxSvrGet() & 0x000000FF);
        if (chipRev != 0x10) /* fixed in P5040 silicon 1.1 */
            {
            vxEnableBP();
            }
        else
...
void sysUsDelay
    ...
    oneUsDelay = (((sysClkFreq >> 4) + 1000000) / 1000000);
...
void coreLbcShow(void)
    ...
    tmp = * (VINT32 *) P5_BR0(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR0(CCSBAR);
    printf("Local bus BR0 = 0x%x\tOR0 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR1(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR1(CCSBAR);
    printf("Local bus BR1 = 0x%x\tOR1 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR2(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR2(CCSBAR);
    printf("Local bus BR2 = 0x%x\tOR2 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR3(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR3(CCSBAR);
    printf("Local bus BR3 = 0x%x\tOR3 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR4(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR4(CCSBAR);
    printf("Local bus BR4 = 0x%x\tOR4 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR5(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR5(CCSBAR);
    printf("Local bus BR5 = 0x%x\tOR5 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR6(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR6(CCSBAR);
    printf("Local bus BR6 = 0x%x\tOR6 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_BR7(CCSBAR);
    tmp2 = * (VINT32 *) P5_OR7(CCSBAR);
    printf("Local bus BR7 = 0x%x\tOR7 = 0x%x\n", tmp, tmp2);

    tmp = * (VINT32 *) P5_LBCR(CCSBAR);
    printf("Local bus LBCR = 0x%x\n", tmp);

    tmp = * (VINT32 *) P5_LCRR(CCSBAR);
...
/* INT LINE TO IRQ assignment for p5040ds board. */
...
void sysAmpCpuEnable
    ...
    *((volatile int *)CPUn_LOCAL_MEM_RPN(cpu))  = usrWrloadBiasLowGet (cpu);
    ...
    if((*P5_BRR(CCSBAR) & cpuBit) == 0)
        {
        *P5_CTBENR(CCSBAR) |= cpuBit;
        *P5_BRR(CCSBAR) |= cpuBit;
        _WRS_ASM("sync;isync");
        }
...
* core cluster 3 PLL frequency divided by two. For the P5040DS board,
...
LOCAL UINT32 sysFmanClkFreqGet (void)
    ...
    if ((*P5_RCWSR(CCSBAR,7) & 0x60000000) == 0)
    ...
    /* Now get the core cluster 2 PLL multiplier */

    ccRatio = P5_CCN_PLL_RAT(CCSBAR, 1);
...
void sysCpusetReset
    ...
    for (i = 1; i < MAX_CPUS; i++)
        ...
        if (PHYS_CPUSET_ISSET(physCpuset, i))
            ...
            *P5_EPICCTPRn(CCSBAR, i) = EPIC_CTPR_MAX;
    ...
    *P5_CNAPCRL(CCSBAR) |= physCpuset;

    _WRS_ASM("sync;isync");

    /* reset cores using PIR */

    *P5_EPICPIR(CCSBAR) |= physCpuset;
