...
--------------------
01b,10oct08,x_s fixed the wrong parameter of flashPtr in function cfiscsErase and 
                code clean.(WIND00135974)
01a,18jun07,x_s initial creation, original version from M-Systems.
*/
...
LOCAL FLStatus cfiscsGetChipCount
	...
	DEBUG_PRINT("Debug: Looking for Flash device at base vol.chipSize 0x%x vol.noOfChips %x\n", vol.chipSize, vol.noOfChips);
#endif
...
LOCAL FLStatus cfiscsWordWrite
    ...
    if (flWriteProtected(vol.socket))
    	{
#ifdef DEBUG_PRINT
	    DEBUG_PRINT("begin cfiscsWordWrite   1.\n");
#endif	
		return flWriteProtect;
    	}
	...
	{
    *flashPtr = (USHORT)(CLEAR_LOCK_BIT << 8 | CLEAR_LOCK_BIT);
    *flashPtr = (USHORT)(CONFIRM_ERASE << 8 | CONFIRM_ERASE);
	*flashPtr = (USHORT)(READ_ARRAY << 8 | READ_ARRAY);
	}
	
    /* we assume that the interleaving is 1. */
    for (from = 0; (from < length / 2) && (status == flOK); from += 1) 
	{
		flashPtr = (FlashWPTR)flMap(vol.socket, address + from * 2);
	
	    *flashPtr = (USHORT)(PROGRAM << 8 | PROGRAM);/*PROGRAM ;*/
	    *flashPtr = * ( (FlashWPTR) (temp + from * 2) ) ;

		    
	    while (!(*flashPtr & (SR_READY << 8|SR_READY)))
		      ;
		if (!(*flashPtr & (SR_READY << 8|SR_READY))) 
	    {
#ifdef DEBUG_PRINT
	    DEBUG_PRINT("Debug: timeout error in CFISCS write.\n");
#endif
	    ...
	    {
#ifdef DEBUG_PRINT
	    DEBUG_PRINT("Debug: CFISCS write error *flashPtr  %x.\n", *flashPtr );
#endif
	    status = (*flashPtr & SR_VPP_ERROR) ? flVppFailure : flWriteFault;
	    *flashPtr = (USHORT)(CLEAR_STATUS << 8 | CLEAR_STATUS); 
	    }
		*flashPtr = (USHORT)(READ_ARRAY << 8 | READ_ARRAY);/*READ_ARRAY; */
    }

	flashPtr = (FlashWPTR)flMap(vol.socket, address);
	
	*flashPtr = (USHORT)(CLEAR_LOCK_BIT << 8 | CLEAR_LOCK_BIT);
	*flashPtr = (USHORT)(CONFIRM_SET_LOCK_BIT << 8 | CONFIRM_SET_LOCK_BIT); 
	*flashPtr = (USHORT)(READ_ARRAY << 8 | READ_ARRAY);
...
#ifdef DEBUG_PRINT
	    DEBUG_PRINT("begin cfiscsWordWrite.%x flashPtr %x buffer %x length  %x mode %x \n", vol, flashPtr, buffer, length, mode);
#endif

#ifdef DEBUG_PRINT
{
	int i = 0;
	FlashWPTR fT = flashPtr;
	USHORT * bT = (USHORT *)buffer;
		
    for (i = 0; i < length/2; i ++)
    {
        if (length == 8)
    		printf("The fT is %x The buffer is %x\r\n", (USHORT)(*fT++), (USHORT)(*bT++));
    }
}
#endif	
    /* verify the data */
    if (status == flOK && tffscmp((void FAR1 *) flashPtr,buffer,length/2)) 
	{
#ifdef DEBUG_PRINT
	DEBUG_PRINT("Debug: CFISCS write failed in verification.\n");
#endif
...
LOCAL FLStatus cfiscsErase
    ...
    for (iBlock = firstEraseBlock;
        ...
        {
            {
            flashPtr[ix * thisCFI->interleaveFactor] = (char)(CLEAR_LOCK_BIT << 8 |
                                                              CLEAR_LOCK_BIT);
            flashPtr[ix * thisCFI->interleaveFactor] = (char)(CONFIRM_ERASE << 8 |
                                                              CONFIRM_ERASE);
            flashPtr[ix * thisCFI->interleaveFactor] = (char)(READ_ARRAY << 8 |
                                                              READ_ARRAY);
            }

            flashPtr[ix * thisCFI->interleaveFactor] = (char)(SETUP_BLOCK_ERASE << 8 |
                                                              SETUP_BLOCK_ERASE);
            flashPtr[ix * thisCFI->interleaveFactor] = (char)(CONFIRM_ERASE << 8 |
                                                              CONFIRM_ERASE);

            while (!(flashPtr[ix * thisCFI->interleaveFactor] & (SR_READY << 8|
                                                                  SR_READY)));
            do
            {
                finished = TRUE;
                ...
                if (!(flashPtr[ix * thisCFI->interleaveFactor] & SR_READY))
                ...
                else 
                   {
                    if (flashPtr[ix * thisCFI->interleaveFactor] & WSM_ERROR) 
                    {
                        ...
                        flashPtr[ix * thisCFI->interleaveFactor] = (char)(CLEAR_LOCK_BIT << 8 |
                                                                          CLEAR_LOCK_BIT);
                        flashPtr[ix * thisCFI->interleaveFactor] = (char)(CONFIRM_SET_LOCK_BIT << 8 |
                                                                          CONFIRM_SET_LOCK_BIT);
                        flashPtr[ix * thisCFI->interleaveFactor] = (char)(READ_ARRAY << 8 |
                                                                          READ_ARRAY);
                    }
            }
...
#ifdef DEBUG_PRINT
        DEBUG_PRINT ("Debug: detected one 16 bit device vol.type %x\n", vol.type);
#endif
	goto getCFI;
	}
...
getCFI:
    ...
    {
#ifdef DEBUG_PRINT
    DEBUG_PRINT("thisCFI->altCommandSetId........\n");
#endif
        return flUnknownMedia;
    }
    ...
    {
#ifdef DEBUG_PRINT
    DEBUG_PRINT ("cfiscsGetChipCount erroe----------------\n");
#endif

	return flUnknownMedia;
    }
#ifdef DEBUG_PRINT
    DEBUG_PRINT ("Debug: No of chips detected is %d thisCFI->vpp %x\n", vol.noOfChips, thisCFI->vpp);
#endif
