/* romInit.s - ROM bootcode for cds85xx BSP */

/*
 * Copyright (c) 2002-2004, 2006, 2009-2010 Wind River Systems, Inc.
 *
...
--------------------
01q,29dec10,y_y  undef INCLUDE_ECC. (WIND00246676)
01p,11jan10,pch  Add symbolic definitions for TIMING_CFG_1
01o,08dec09,pch  CQ195058: Clean up DDR setup
01n,18nov09,pch  Port ECC scrub from ads85xx/romInit.s vn 01n,27sep06,dtr
01m,04sep04,jln  supported ads85xx
...
/* 
 * cause some cds85xx's sdram does not support
 * hardware ecc, so we undef it here
 */
...
/* This macro shifts an unsigned value, v, right s places, without
 * sign-extending the result as would happen with (v >> s).
 */
#define SHIFT_UNSIGNED(v, s) \
        (((v & 0x7fffffff) >> s) | ((v >> s) & (1 << (31 - s))))
...
FUNC_LABEL(_resetEntry)
	...
	WRITEADR(r6,r7,0xFF700000,SHIFT_UNSIGNED(CCSBAR, 12))
	sync
	isync

	/* Initialise the Local Bus Controller */

	WRITEADR(r6,r7,M85XX_LCRR(CCSBAR),0x00030008)
...
dllDone:
	...
	WRITEOFFSET(r6,r7,(CS2_BNDS), 0x00000000)   /* bank 2: not present */
	WRITEOFFSET(r6,r7,(CS3_BNDS), 0x00000000)   /* bank 3: not present */

	/* DIMM 0, bank 0 */
	WRITEOFFSET(r6,r7,(CS0_CONFIG),					     \
		CS_ENABLE | DDR_SDRAM_ROW_ADRS_BITS | DDR_SDRAM_COL_ADRS_BITS)
	...
	WRITEOFFSET(r6,r7,(CS2_CONFIG), 0x00000000) /* bank 2: disabled */
	WRITEOFFSET(r6,r7,(CS3_CONFIG), 0x00000000) /* bank 3: disabled */

	/* Unfortunately, all but CASLAT here are received wisdom. */
#define	TIMING_CFG_1_VAL						\
	 ((3 << DDR_SDRAM_TIMING_CFG_1_PRETOACT_SHIFT)			\
	| (7 << DDR_SDRAM_TIMING_CFG_1_ACTTOPRE_SHIFT)			\
	| (3 << DDR_SDRAM_TIMING_CFG_1_ACTTORW_SHIFT)			\
	| (DDR_SDRAM_CASLAT << DDR_SDRAM_TIMING_CFG_1_CASLAT_SHIFT)	\
	| (4 << DDR_SDRAM_TIMING_CFG_1_REFREC_SHIFT)			\
	| (3 << DDR_SDRAM_TIMING_CFG_1_WRREC_SHIFT)			\
	| (2 << DDR_SDRAM_TIMING_CFG_1_ACTTOACT_SHIFT)			\
	| (1 << DDR_SDRAM_TIMING_CFG_1_WRTORD_SHIFT))
	WRITEOFFSET(r6,r7,(TIMING_CFG_1), TIMING_CFG_1_VAL)

	WRITEOFFSET(r6,r7,(TIMING_CFG_2), 0x00000800)
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CFG),
		    DDR_SDRAM_CFG_SREN | DDR_SDRAM_TYPE_DDR)

	WRITEOFFSET(r6,r7,(DDR_SDRAM_MODE_CFG), 0x00000062)
	WRITEOFFSET(r6,r7,(DDR_SDRAM_INTERVAL), 0x05200000)

	/*
	 * The 1/2 cycle clock delay specified here is received wisdom for
	 * the cds85xx with its supplied DIMM, and may need to be changed
	 * if porting to other boards and/or using a different DIMM.
	 */

	WRITEOFFSET(r6,r7,(DDR_SDRAM_CLK_CNTL),
		  DDR_SDRAM_CLK_CNTL_SS_ENABLE | DDR_SDRAM_CLK_CNTL_CLK_ADJST_2)
...
1:
	...
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CFG),
	     DDR_SDRAM_CFG_MEM_ENABLE | DDR_SDRAM_CFG_SREN | DDR_SDRAM_TYPE_DDR)
...
eccDelayLoop:
	...
	WRITEOFFSET(r6,r7,(DDR_SDRAM_CFG),
	   ...
	   | DDR_SDRAM_CFG_ECC_ENABLE | DDR_SDRAM_TYPE_DDR)
#endif	/* INCLUDE_ECC */
	isync
	sync
	mbar 0
	...
	WRITEADR(r6,r7,M85XX_BR1(CCSBAR),FLASH1_BASE_ADRS|0x01001 )

	WRITEADR(r6,r7,M85XX_OR1(CCSBAR),FLASH1_ADRS_MASK|0x6e61 )
	...
	WRITEADR(r6,r7,M85XX_LAWBAR1(CCSBAR), \
		 SHIFT_UNSIGNED(LBC_CS3_LOCAL_ADRS, LAWBAR_ADRS_SHIFT) )
...
SdramWrLoop2:
	...
	/*
	 * TLB1 #2.  Main SDRAM - Cached with coherence
	 *           LOCAL_MEM_LOCAL_ADRS -> LOCAL_MEM_LOCAL_ADRS + LOCAL_MEM_SIZE
	 * Attributes: UX/UW/UR/SX/SW/SR
	 */
	...
	/*
	 * TLB1 #3.  CCSRBAR - guarded 0xfe000000 -> 0xfe000000.
	 * 16MB
	 * Attributes: SX/SW/SR
	 */
	...
	li      r30, 0
	li      r31, 0
	lis     r5, HI((LOCAL_MEM_SIZE >> 4) & 0x0fffffff) /* unsigned */
	mtctr   r5
	bdnz    eccLoad
eccLoad:
	stmw    r24,0(r6)
	...
	/*
	 * Scrub complete.  Clear error detection
	 * register & single-bit error counter.
	 */
	...
	sync
	mbar 0
	WRITEOFFSET(r6,r7,(DDR_ERR_DETECT),
		    DDR_ERR_DETECT_MME | DDR_ERR_TYPE_MBE
                  | DDR_ERR_TYPE_SBE | DDR_ERR_TYPE_MSE)
	WRITEOFFSET(r6,r7,(DDR_ERR_SBE), 1 << DDR_ERR_SBE_SBET_SHIFT)

	isync
	sync
	mbar 0

	/* could set MBEE, SBEE, and/or MSEE here */
	WRITEOFFSET(r6,r7,(DDR_ERR_INT_EN),
		    DDR_ERR_TYPE_MBE | DDR_ERR_TYPE_SBE)
	isync
	sync
	mbar 0
