...
//		*pOffset = ifUnit * (MAC_ADRS_LEN + 2); /* Round up to 32 bit word */   sjy 2013.06.20 modified
		/**ppEnet  = (char*)glbEnetAddr[ifUnit + MAC_OFFSET_MOTTSEC];*/
		/* *ppEnet  = (char*)glbEnetAddr[sysGetCpuID() + MAC_OFFSET_MOTTSEC];*//*hdz*/
		*ppEnet  = (char*)glbEnetAddr[ MAC_OFFSET_MOTTSEC+ifUnit]; 
	}
	else
		return(ERROR);
...
#if (NV_RAM_SIZE != NONE)
	/* sysNvRamGet (pMac, ifMacAddrLen, NV_MAC_ADRS_OFFSET+offset);*/
	/*  pMac = (char*)(BOOT_LINE_ADRS + NV_MAC_ADRS_OFFSET);*/
	pMac = (char*)(NV_RAM_ADRS + NV_PAREM_OFFSET + NV_MAC_ADRS_OFFSET+NV_BOOT_OFFSET+(MAC_ADRS_LEN+2)*(sysGetCpuID()*4+ifUnit));
//	printf("+++++++mac address store address 0x%x\n",pMac);
	if (( memcmp(pMac, sysInvalidAddr[0], MAC_ADRS_LEN) != 0 )
			&&( memcmp(pMac, sysInvalidAddr[1], MAC_ADRS_LEN) != 0 ))
	{
		memcpy (pEnet, pMac, ifMacAddrLen);
		macFromNvram = TRUE;
	}

#endif /* (NV_RAM_SIZE != NONE) */
	if (( memcmp(pEnet, sysInvalidAddr[0], MAC_ADRS_LEN) == 0 )
			||( memcmp(pEnet, sysInvalidAddr[1], MAC_ADRS_LEN) == 0 ))
		return(ERROR);

	if(FALSE == macFromNvram)
	{
		*(pEnet+5) = ifUnit;
		*(pEnet+4) = sysGetCpuID();
//		*(pEnet+3) = sysSlotNumGet();
	}
...
)
{
	...
	/*sysNvRamGet (pEnet, ifMacAddrLen, NV_BOOT_OFFSET+NV_MAC_ADRS_OFFSET+offset);*/
	sysNvRamGet (pEnet, ifMacAddrLen, NV_BOOT_OFFSET + NV_PAREM_OFFSET + NV_MAC_ADRS_OFFSET+(MAC_ADRS_LEN+2)*(sysGetCpuID()*4+ifUnit)+offset);
//	sysNvRamGet (pEnet, ifMacAddrLen, NV_MAC_ADRS_OFFSET+(MAC_ADRS_LEN+2)*sysGetCpuID());
...
//	*(pEnet+4) = 0xbb;
//	*(pEnet+3) = 0xaa;
	/* sysNvRamSet ((char *)ifMacAddr, ifMacAddrLen,NV_BOOT_OFFSET+NV_MAC_ADRS_OFFSET+offset);*//*hdz*/
//	sysNvRamSet ((char *)ifMacAddr, ifMacAddrLen, NV_BOOT_OFFSET + NV_MAC_ADRS_OFFSET+(MAC_ADRS_LEN+2)*sysGetCpuID()+offset);

//	d((NV_RAM_ADRS + NV_PAREM_OFFSET + NV_MAC_ADRS_OFFSET+NV_BOOT_OFFSET+(MAC_ADRS_LEN+2)*(sysGetCpuID()*4+ifUnit)),8,4);
//	d(pEnet,8,4);
//	d(ifMacAddr,8,4);
	memcpy((char*)(NV_RAM_ADRS + NV_PAREM_OFFSET + NV_MAC_ADRS_OFFSET+NV_BOOT_OFFSET+(MAC_ADRS_LEN+2)*(sysGetCpuID()*4+ifUnit)),(char *)ifMacAddr,ifMacAddrLen);
//	printf("inaaaa sysNetMacAddrSet 0x%x 0x%x\n",NV_RAM_ADRS +NV_PAREM_OFFSET + NV_MAC_ADRS_OFFSET+NV_BOOT_OFFSET+(MAC_ADRS_LEN+2)*sysGetCpuID(),ifUnit);
#endif /* (NV_RAM_SIZE != NONE) */
