...
--------------------
01g,02feb12,y_c  move lable checkStop from function resetEntry to romInit
                 to fix the make release error. (WIND00331112)
01f,31oct11,y_c  add fix for MPC8548 DDR20 errata. (WIND00292034)
01e,11may10,liu  remove DDR IO receiver biasing initialize. (WIND00186659)
...
#define SETBIT(reg1,reg2,addr32,maskbit) \
        READADR(reg1,reg2,addr32); \
        lis    reg1,HI(0xffffffff); \
        ori    reg1,reg1,LO(0xffffffff); \
        rlwimi  reg2, reg1, 0, maskbit, maskbit; \
        lis    reg1, HI(addr32); \
        ori    reg1, reg1, LO(addr32); \
        stw reg2, 0(reg1)

#define CLRBIT(reg1,reg2,addr32,maskbit) \
        READADR(reg1,reg2,addr32); \
        rlwinm  reg2, reg2, 0, maskbit+1, maskbit-1; \
        lis    reg1, HI(addr32); \
        ori	reg1, reg1, LO(addr32); \
        stw reg2, 0(reg1)
...
finalDDRInit:
  	...
  	isync
    /* Freescale Errata DDR20 Workaround, Option 1 */
#ifdef INCLUDE_FSL_DDR_20_WORKAROUND
    /* Set reserved bit EEBACR[3] at offset 0x1000 */

    SETBIT(r6,r7,EEBACR(CCSBAR),3);

    /* Set DDR_SDRAM_CFG_2[D_INIT] */

    SETBIT(r6,r7,DDRBA + DDR_SDRAM_CFG_2,27);

    /* Set D3[21] */

    SETBIT(r6,r7,DDRBA + 0xf08,21);
#endif /* INCLUDE_FSL_DDR_20_WORKAROUND */
    li      r4,0x2000
    mtctr   r4
ddrDelay:
    nop
    ...
    /* Freescale Errata DDR20 Workaround, Option 1 */
#ifdef INCLUDE_FSL_DDR_20_WORKAROUND
    /* Poll DDR_SDRAM_CFG_2[D_INIT] until it is cleared by hardware */

Poll_CFG2_Bit:
    READADR(r6,r7,DDRBA + DDR_SDRAM_CFG_2)
    lis	r6, HI(0x10); 
    ori	r6, r6, LO(0x10);
    and      r7,r7,r6
    cmpwi   r7, 0x10
    ...
    CLRBIT(r6,r7,DDRBA + 0xf08,21);

    /*Set D2[21]*/

    SETBIT(r6,r7,DDRBA + 0xf04,21);

    /* Poll on D2[21] until it is cleared by hardware */

Poll_D2_Bit:
    READADR(r6,r7,DDRBA + 0xf04)
    lis	r6, HI(0x400); 
    ori	r6, r6, LO(0x400);
    and      r7,r7,r6
    cmpwi   r7, 0x400
...
Poll_CFG2_Bit_ECC:
    ...
    and      r7,r7,r6
    cmpwi   r7, 0x10
    beq     Poll_CFG2_Bit_ECC
#endif /* INCLUDE_DDR_ECC */

    /* Clear reserved bit EEBACR[3] at offset 0x1000 */

    CLRBIT(r6,r7,EEBACR(CCSBAR),3);
#endif /* INCLUDE_FSL_DDR_20_WORKAROUND */
