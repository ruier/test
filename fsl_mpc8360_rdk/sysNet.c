...
01c,07jun12,mpc  Fixed potential memory corruption caused by sysNvRamGet().
                 (WIND00101057)
01e,22jun08,y_w  Switch from hEnd qeFcc driver to VxBus UCC driver
01d,08jan08,z_l  Use default value if MAC get fail.
...
UCHAR sysFccEnetAddrDefault[12] =
    {0x00, 0x04, 0x9f, 0x00, 0xa0, 0xa0, 0x00, 0x04, 0x9f, 0x00, 0xa0, 0xa1};
...
STATUS sysEnetAddrGet
    ...
    if(status != OK)
        {
        bcopy ((char *)&sysFccEnetAddrDefault[(unit&0x1)*MAC_ADRS_LEN],
               (char *)pAddr, MAC_ADRS_LEN);
