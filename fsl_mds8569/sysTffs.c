...
--------------------
01a,10aug09,x_z  derived from version 01a of amcc_kilauea/sysTffs.c.
*/
...
Note: tffsBootImagePut is not supported because the bootrom area must be
reserved at the end of the FLASH.
...
if INCLUDE_MTD_USR is defined, it includes:
    sysMtd.c -      MTD driver

if DOC is not defined, it includes:
    tffsConfig.c -  TrueFFS configuration file for VxWorks

INCLUDE FILES: taskLib.h flsocket.h tffsDrv.h
*/
...
#define INCLUDE_MTD_USR                 /* use special MTD driver */

#ifdef INCLUDE_MTD_USR
#   include "sysMtd.c"
#   define  MTD_USR_IDENTIFY   s29GlIdentify
...
/*******************************************************************************
*
* sysTffsInit - initialize for TFFS on board level
*
...
/*******************************************************************************
*
* rfaRegister - install necessary functions for the Flash RFA
*
...
LOCAL void rfaSetWindow
    ...
    vol.window.baseAddress = (FLASH_TFFS_ADRS) >> 12;
...
/*******************************************************************************
*
* sysTffsProgressCb - print format process info
*
* This routine prints format process info after erasing each unit.
*
* RETURNS: flOK to allows formatting to continue
*/

LOCAL int sysTffsProgressCb
    (
    int totalUnitsToFormat,         /* total number of erase units to format */
    int totalUnitsFormattedSoFar    /* number erased so far                  */
    )
    {
...
*
* This routine formats the flash memory.
*
...
STATUS sysTffsFormat (void)
    ...
    /*
     * tffsBootImagePut is not supported because the bootrom area must be
     * reserved at the first of the FLASH, But the rom start address must
     * be located at the end of the FLASH.
     */

    tffsDevFormatParams params =
    {
    {0x00000l, 99, 1, 0x10000l, PROGRESS_CB, {0,0,0,0}, NULL, 2, 0, NULL},
    FTL_FORMAT_IF_NEEDED
    };

    /* make sure we are not called in the interrupt context */

    if (INT_CONTEXT ())
        {
        if (_func_logMsg != NULL)
            _func_logMsg ("Can't call sysTffsFormat() from interrupt level!\n",
                          1,2,3,4,5,6);
        return flIncomplete;
        }
