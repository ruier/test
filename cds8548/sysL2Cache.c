...
--------------------
01d,10feb09,b_m  make l2CacheLineNum LOCAL.
...
LOCAL void sysL2CacheEnable(int cacheType);
LOCAL void sysL2CacheDisable(int cacheType);
LOCAL void sysL2CacheFlush(int cacheType);

IMPORT void sysCacheFlush(int, char *, int);

STATUS sysL2ExtWriteBufferAlloc (char * adrs, UINT size, BOOL lock);

/* 512/64  == (Min Cache Size / L2 Cache Line Size) */

LOCAL int l2CacheLineNum = ((4096 + 8192) * L2_CACHE_SIZE); 
