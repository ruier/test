...
--------------------
01c,17mar08,mdg  Updated for the mv7100.
01b,29aug05,efb  Modified for MV3100 application.
...
/* Disable swapping macros since BSP already does PCI byte swaps */
...
#define FROM_LITTLEW(x) (x)
#define FROM_LITTLEL(x) (x)
#define TO_LITTLEW(x) (x)
#define TO_LITTLEL(x) (x)
...
#undef  USB_PCI_MEMIO_OFFSET
#undef  USB_PCI_IO_OFFSET
#undef  USB_PCI_MEM_OFFSET

#define USB_PCI_IO_OFFSET   PCI_IO_LOCAL_START
...
#define USB_PCI_IN_BYTE(a)  sysInByte ((a))
#define USB_PCI_IN_WORD(a)  sysInWord ((a))
#define USB_PCI_IN_DWORD(a) sysInLong ((a))
#define USB_PCI_OUT_BYTE(a,v)   sysOutByte ((a), (v))
#define USB_PCI_OUT_WORD(a,v)   sysOutWord ((a), (v))
#define USB_PCI_OUT_DWORD(a,v)  sysOutLong ((a), (v))
...
IMPORT STATUS sysPciConfigWrite (int busNo, int deviceNo, int funcNo,
...
BOOL usbPciClassFind
    ...
    if (CPU1CHECK)
        ...
        /* Core 1 does not control the USB busses */
...
STATUS usbPciIntConnect
    ...
    return ERROR;
    printf("Connecting interrupt on %d\n",intNo);
