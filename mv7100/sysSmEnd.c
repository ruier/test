...
--------------------
01f,22dec09,e_d  Modified the include file
01e,28mar08,mdg  Fixed issue where no specified backplane address would
                  lock up bootrom.
01d,21dec06,dtr  Fix compiler error.
01c,22sep06,mze  adding SM_MAX_PKTS and SM_PKTS_SIZE for WIND00058841
01b,30aug04,j_b  remove INCLUDE_NETWORK dependencies (SPR #99747)
01a,21jan04,jln  created
*/
    ...
    return (NULL);
        ...
        sprintf (cp, "0x%x:0x%x:0x%x:%d:%d:%d:%d:%d:%d:%d:0x%x:0x%x:0x%x:"
                ...
                (UINT32)SM_ANCHOR_ADRS,      /* ANCHOR address */
                 ...
                 SM_PKTS_SIZE,              /* Max bytes per packet, 0default */
        ...
        /* If master CPU and no backplane address, then don't try to load */

        if (sysProcNumGet() == 0 && bootParams.bad[0] == EOS)
            {
            if ((configFlag &
                 (SMEND_PROXY_SERVER_FLAG | SMEND_PROXY_DEFAULT_ADDR_FLAG)) !=
                 (SMEND_PROXY_SERVER_FLAG | SMEND_PROXY_DEFAULT_ADDR_FLAG))
                {
                printf ("Error: sysSmEndLoad() needs to specify master IP address!\n");
                return(NULL);
                }
            }
