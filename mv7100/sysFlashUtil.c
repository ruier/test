...
--------------------
01c,25apr08,mdg  Compiler warnings.
...
for the AMD S29GL512/256/128N MirrorBit Flash on the MVME7100.  It provides an
...
LOCAL STATUS sysFlashUtilConfig
    ...
    switch (secDevCode)
        {
        case FMID_AMD_GL128:
        flashDev->pDeviceName = (UCHAR *) "AMD S29GL128N MirrorBit";
        break;
        case FMID_AMD_GL256:
            flashDev->pDeviceName = (UCHAR *) "AMD S29GL256N MirrorBit";
        break;
        default:        /* and FMID_AMD_GL512 */
        flashDev->pDeviceName = (UCHAR *) "AMD S29GL512N MirrorBit";
        break;
        }
...
LOCAL STATUS sysFlashUtilInit (void)
    ...
    if (! flashInitFlag)
        {
        for (bank = STARTING_FLASH_BANK;
             bank < NUM_FLASH_BANKS+STARTING_FLASH_BANK;
             bank++)
            {
            pFlashDev = &sysFlashDevs[bank-STARTING_FLASH_BANK];
