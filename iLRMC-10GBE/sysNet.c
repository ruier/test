...
/***********************************************************************/
/*add by xm for crate device ID*/
STATUS	sysCrateIDSet(UINT32 uiNum,UINT32 uiDeviceTypeId)
{
	if(ERROR == sysNvRamSet(&uiNum, sizeof(UINT32), CRATE_ID_OFFSET))
	{
		logMsg("set crate ID failed!\n",0,0,0,0,0,0);
		return ERROR;
	}
	
	if(ERROR == sysNvRamSet(&uiDeviceTypeId, sizeof(UINT32), CRATE_ID_OFFSET + 4))
	{
		logMsg("set crate Device Ty[e ID failed!\n",0,0,0,0,0,0);
		return ERROR;
	}
...
STATUS sysCrateIDGet(UINT32 *uiDeviceNum,UINT32 *uiDeviceTypeId)
{
	if(ERROR == sysNvRamGet(uiDeviceNum, 8, CRATE_ID_OFFSET))
	{
		logMsg("get crate ID failed!\n",0,0,0,0,0,0);
		return ERROR;
	}
	if(ERROR == sysNvRamGet(uiDeviceTypeId, 8, CRATE_ID_OFFSET + 4 ))
	{
		logMsg("get uiDeviceTypeId ID failed!\n",0,0,0,0,0,0);
		return ERROR;
	}
	return OK;
}
