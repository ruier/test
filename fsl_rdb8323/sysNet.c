...
01e,02jun09,e_d  code cleanup
01d,17mar08,wap  Switch from hEnd qeFcc driver to VxBus UCC driver
...
STATUS sysMacOffsetGet
    ...
    if ( strcmp(ifName,"qefcc") == 0 )
        ...
        *ppEnet  = (char*)glbEnetAddr[ifUnit + MAC_OFFSET_UCC];
        }
    else
        return(ERROR);
...
int sysNetMacAddrClear
    ...
    sysNvRamSet ((char *)ifMacAddr, MAC_ADRS_LEN, NV_MAC_ADRS_OFFSET+offset);
...
* used by each individual QEFCC device unit.  This routine must copy
