/*  sysMpc8540I2c.c -  Low Level mpc8540 I2C Routines */
...
#define I2C_READ08		mpc8540I2cRead08
#define I2C_WRITE08		mpc8540I2cWrite08
...
STATUS sys8540I2cStart (UINT32 base);
STATUS sys8540I2cStop (UINT32 base);
STATUS sys8540I2cReadN (UINT32 base, UINT8 * pDataChar, UINT32 numBytes);
STATUS sys8540I2cRead (UINT32 base, UINT8 * pDataChar); 			
STATUS sys8540I2cWriteN (UINT32 base, UINT8 * pDataChar, UINT32 numBytes);
STATUS sys8540I2cWrite (UINT32 base, UINT8  pDataChar);
STATUS sys8540I2cAckIn (UINT32 base);
STATUS sys8540I2cAckOut (UINT32 base);
STATUS sys8540I2cKnownState (UINT32 base);
LOCAL UINT8 mpc8540I2cRead08 (UINT32 base, UINT32 offset);
LOCAL void mpc8540I2cWrite08 (UINT32 base, UINT32 offset, UINT8 value);
...
/*******************************************************************************
*
* sys8540I2cStart - perform an I2C start cycle on the I2C bus
*
...
STATUS sys8540I2cStart 
    (
    UINT32 base		/* Base of mpc8540 register set */
    )
    {
    UINT32 timeOutCount;
    UINT8  reg08;
...
/*******************************************************************************
*
* sys8540I2cStop - perform an I2C stop cycle on the I2C bus
*
...
STATUS sys8540I2cStop 
     ...
     * Turn off MTX bit(which places the mpc8540 interface into receive mode
...
/*******************************************************************************
*
* sys8540I2cReadN - perform an I2C read cycle of N bytes.
*
...
/*******************************************************************************
*
* sys8540I2cRead - perform an I2C read cycle on the I2C bus.
*
...
STATUS sys8540I2cRead
    (
    UINT32 base,		/* Base of mpc8540 register set */
    UINT8  * pDataChar	        /* Read character returned here */
    ) 			
    {
    return (sys8540I2cReadN (base, pDataChar, 1));
    }

/*******************************************************************************
*
* sys8540I2cWriteN - perform I2C write cycles of N bytes on I2c bus.
*
...
/*******************************************************************************
*
* sys8540I2cWrite - perform an I2C write cycles on I2c bus.
*
...
STATUS sys8540I2cWrite
    ...
    stat = sys8540I2cWriteN (base, &pDataChar, 1);
    return (stat);
    }

/*******************************************************************************
*
* sys8540I2cAckIn - Perform an "acknowledge-in" cycle on the I2C bus
*
...
/*******************************************************************************
*
* sys8540I2cAckOut - Perform an "acknowledge-out" cycle on the I2C bus
*
* This routine simply returns OK.  The mpc8540 does not require this routine,
...
STATUS sys8540I2cAckOut 
    (
    UINT32 base  ARG_UNUSED 	/* Base of mpc8540 register set */
    )
    {
    return(OK);
    }
...
/******************************************************************************
*
* sys8540I2cKnownState - initialize the I2C bus to a known state
*
...
/*******************************************************************************
*
* mpc8540I2cRead08 - Read one 8-bit value from I2C register 
*
...
/*******************************************************************************
*
* mpc8540I2cWrite08Res - Write to I2C register, preserve reserved bits.
*
