...
--------------------
01b,07may13,d_l Fixed build warnings and prevent errors.
01a,17dec04,cak Ported from 01d, mv6100.
*/
...
void sysTempeWinShow (void)
	...
	{
        getOutWinVals (i, &vo);
        if (vo.otatImg & (UINT32)(1<<31))	/* Enabled so show it */
	...
	{
	getInWinVals (i, &vi);
        if (vi.itatImg & (UINT32)(1<<31))	/* Enabled so show it */
	    sysTempeInWinShow(i);
	else
