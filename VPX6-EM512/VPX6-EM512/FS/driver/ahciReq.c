...
VOID ahciRWSectorDone(VOID *pstOrgReq)
{
    ...
    ((AHCI_DRV_S*)pstAhciReq->pvDrv)->uiActiveNCQ &= (~(1 << pstAhciReq->ucTag));
...
/*****************************************************************************
 function     : ahciFillTrimFPDMA
 description  : fill trim fpdma queue fis(sata definition command)
*****************************************************************************/
STATUS ahciFillTrimFPDMA(AHCI_REQ_S *pstReq)
{
    UINT32     uiSecCnt = 0;
    ...
    if(uiSecCnt > 0xffff)
    {
        AHCI_LOG(AHCI_DBG_ERR,"trim transfer size %d overflow\n",uiSecCnt,
                0,0,0,0,0);
        return ERROR;
    }
    ...
    pstReq->stFis.ucFisType = SAS_ATA_FIS_H2D;
#if 0
    pstReq->stFis.ucCmd = ATA_CMD_FPDMA_SEND;
    pstReq->stFis.ucSecCntExp = ATA_SUBCMD_FPDMA_SEND_DSM & 0x1F;
    pstReq->stFis.ucSecCnt = pstReq->ucTag << 3;
	pstReq->stFis.ucFeaturesExp = 0;
  	pstReq->stFis.ucFeature = 1;/*seccnt 1*/
    pstReq->stFis.ucAuxiliary = 1;
	pstReq->stFis.ucFlags = 0x80;
...
#else
	pstReq->stFis.ucFlags = 0x80;
	pstReq->stFis.ucCmd = 0x6;
  	pstReq->stFis.ucFeature = 1;/*TRIM*/
	pstReq->stFis.ucDevice = 0xA0;
	pstReq->stFis.ucFeaturesExp = 0;
	pstReq->stFis.ucSecCntExp = 0;
    pstReq->stFis.ucSecCnt = 1;
	pstReq->stFis.ucCtrl = 8;
	...
	pstReq->pucBuf[0] = pstReq->aucLBA[5];
	pstReq->pucBuf[1] = pstReq->aucLBA[4];
	pstReq->pucBuf[2] = pstReq->aucLBA[3];
	pstReq->pucBuf[3] = pstReq->aucLBA[2];
	pstReq->pucBuf[4] = pstReq->aucLBA[1];
	pstReq->pucBuf[5] = pstReq->aucLBA[0];
	pstReq->pucBuf[6] = (UINT8)uiSecCnt;
	pstReq->pucBuf[7] = (UINT8)(uiSecCnt >> 8);

	AHCI_LOG(AHCI_DBG_DATA,"trim transfer FIS 0x%x BUF 0x%x\n",&pstReq->stFis,
                pstReq->pucBuf,0,0,0,0);
...
/*****************************************************************************
 function     : ahciBlkTrim
 description  : ahci trim block
 input        : 
 output       : write data to LBA
 return       : 
*****************************************************************************/
STATUS ahciBlkTrim(AHCI_REQ_S *pstReq)
{
    if(ERROR == ahciFillTrimFPDMA(pstReq))
    {
        return ERROR;
    }
