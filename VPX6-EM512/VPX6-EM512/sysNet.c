...
STATUS sysMacOffsetGet
    ...
    if ( (strcmp(ifName,"mottsec") == 0) || (strcmp(ifName,"motetsec") == 0) )
	{
        ...
        *ppEnet  = (char*)glbEnetAddr[ifUnit + MAC_OFFSET_MOTTSEC + 4 * sysGetCpuID()];
        /*ppEnet  = (char*)glbEnetAddr[sysGetCpuID() + MAC_OFFSET_MOTTSEC];*/
	}
    else
    	return(ERROR);
...
STATUS sysNetMacAddrSet
    ...
    sysNvRamGet (pEnet, ifMacAddrLen, NV_BOOT_OFFSET+NV_MAC_ADRS_OFFSET);
    ...
    sysNvRamSet ((char *)ifMacAddr,ifMacAddrLen,NV_BOOT_OFFSET+NV_MAC_ADRS_OFFSET);
#endif /* (NV_RAM_SIZE != NONE) */
